...
$AdditionalPatchName = "002-libredesk-public-host.patch"
$AdditionalPatchPath = Join-Path $PSScriptRoot $AdditionalPatchName
$UsageExportPatchName = "003-usage-export-reliability.patch"
$UsageExportPatchPath = Join-Path $PSScriptRoot $UsageExportPatchName
...
function Apply-LibredeskPatch {
  if (-not (Test-Path $AdditionalPatchPath)) {
    [Console]::Error.WriteLine("Patch file not found: $AdditionalPatchPath")
    exit 1
  }

  $checkArgs = @("apply", "--check", "--ignore-space-change", "--whitespace=nowarn", $AdditionalPatchPath)
  $checkCode = Test-GitApply -Arguments $checkArgs
  if ($checkCode -eq 0) {
    if ($CheckOnly) {
      Write-Host "Patch can be applied: $AdditionalPatchName" -ForegroundColor Green
      return
    }
    $applyCode = Invoke-GitApply -Arguments @("apply", "--ignore-space-change", "--whitespace=nowarn", $AdditionalPatchPath)
    if ($applyCode -ne 0) {
      [Console]::Error.WriteLine("Failed to apply patch: $AdditionalPatchName")
      exit 1
    }
    Write-Host "Applied patch: $AdditionalPatchName" -ForegroundColor Green
    return
  }

  $reverseCode = Test-GitApply -Arguments @("apply", "--reverse", "--check", "--ignore-space-change", "--whitespace=nowarn", $AdditionalPatchPath)
  if ($reverseCode -eq 0) {
    Write-Host "Patch is already applied: $AdditionalPatchName" -ForegroundColor Yellow
    return
  }

  if ($CheckOnly) {
    [Console]::Error.WriteLine("Patch cannot be applied cleanly: $AdditionalPatchName")
    exit 1
  }

  Write-Host "Patch did not apply cleanly. Trying 3-way apply: $AdditionalPatchName" -ForegroundColor Yellow
  $threeWayCode = Invoke-GitApply -Arguments @("apply", "--3way", "--ignore-space-change", "--whitespace=nowarn", $AdditionalPatchPath)
  if ($threeWayCode -ne 0) {
    [Console]::Error.WriteLine("Patch failed: $AdditionalPatchName")
    exit 1
  }
  Write-Host "Applied patch with 3-way merge: $AdditionalPatchName" -ForegroundColor Green
}

function Apply-UsageExportPatch {
  if (-not (Test-Path $UsageExportPatchPath)) {
    [Console]::Error.WriteLine("Patch file not found: $UsageExportPatchPath")
    exit 1
  }

  $checkArgs = @("apply", "--check", "--ignore-space-change", "--whitespace=nowarn", $UsageExportPatchPath)
  $checkCode = Test-GitApply -Arguments $checkArgs
  if ($checkCode -eq 0) {
    if ($CheckOnly) {
      Write-Host "Patch can be applied: $UsageExportPatchName" -ForegroundColor Green
      return
    }
    $applyCode = Invoke-GitApply -Arguments @("apply", "--ignore-space-change", "--whitespace=nowarn", $UsageExportPatchPath)
    if ($applyCode -ne 0) {
      [Console]::Error.WriteLine("Failed to apply patch: $UsageExportPatchName")
      exit 1
    }
    Write-Host "Applied patch: $UsageExportPatchName" -ForegroundColor Green
    return
  }

  $reverseCode = Test-GitApply -Arguments @("apply", "--reverse", "--check", "--ignore-space-change", "--whitespace=nowarn", $UsageExportPatchPath)
  if ($reverseCode -eq 0) {
    Write-Host "Patch is already applied: $UsageExportPatchName" -ForegroundColor Yellow
    return
  }

  if ($CheckOnly) {
    [Console]::Error.WriteLine("Patch cannot be applied cleanly: $UsageExportPatchName")
    exit 1
  }

  Write-Host "Patch did not apply cleanly. Trying 3-way apply: $UsageExportPatchName" -ForegroundColor Yellow
  $threeWayCode = Invoke-GitApply -Arguments @("apply", "--3way", "--ignore-space-change", "--whitespace=nowarn", $UsageExportPatchPath)
  if ($threeWayCode -ne 0) {
    [Console]::Error.WriteLine("Patch failed: $UsageExportPatchName")
    exit 1
  }
  Write-Host "Applied patch with 3-way merge: $UsageExportPatchName" -ForegroundColor Green
}
...
try {
  ...
  Apply-LibredeskPatch
  Apply-UsageExportPatch

  $checkArgs = @("apply", "--check", "--ignore-space-change", "--whitespace=nowarn", $PatchPath)
  $checkCode = Test-GitApply -Arguments $checkArgs
  ...
  if ($checkCode -eq 0) {
    ...
    $applyCode = Invoke-GitApply -Arguments @("apply", "--ignore-space-change", "--whitespace=nowarn", $PatchPath)
    if ($applyCode -ne 0) {
  ...
  $reverseCode = Test-GitApply -Arguments @("apply", "--reverse", "--check", "--ignore-space-change", "--whitespace=nowarn", $PatchPath)
  if ($reverseCode -eq 0) {
  ...
  $threeWayCode = Invoke-GitApply -Arguments @("apply", "--3way", "--ignore-space-change", "--whitespace=nowarn", $PatchPath)
  if ($threeWayCode -ne 0) {
