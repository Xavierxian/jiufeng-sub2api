...
const navigationPath = computed(() => jiufengNavigationPath(route, authStore))
const navigationTarget = (path: string) => jiufengNavigationTarget(path, authStore)
...
// Lucide's chart-no-axes-combined icon: trend line plus compact bars for
// operations monitoring without the visual weight of a full chart.
const OpsMonitoringIcon = {
  render: () =>
    h(
      'svg',
      { fill: 'none', viewBox: '0 0 24 24', stroke: 'currentColor', 'stroke-width': '1.5' },
...
function buildAdminWorkspaceSections(): WorkspaceSection[] {
  ...
  const sections: WorkspaceSection[] = [
    ...
    makeSection('ops', t('nav.ops'), OpsMonitoringIcon, items, ['/admin/ops', '/admin/usage']),
    ...
    makeSection('accounts', t('nav.accounts'), GlobeIcon, items, ['/admin/accounts', '/admin/plugins']),
...
function isSubnavActive(path: string): boolean {
  return navigationPath.value === path
}
...
function handleSectionClick(section: WorkspaceSection) {
  ...
  if (!target) return
  router.push(navigationTarget(target.path))
...
function isActive(path: string): boolean {
  return navigationPath.value === path || navigationPath.value.startsWith(path + '/')
}
