<template>
  <JFAppLayout>
    <div class="space-y-6">
      ...
      <div class="border border-blue-200 bg-blue-50 px-4 py-3 text-sm text-blue-800 dark:border-blue-900/60 dark:bg-blue-950/30 dark:text-blue-200">
      ...
      <div v-else class="grid grid-cols-1 gap-4 xl:grid-cols-2">
        ...
        >
          <div class="flex flex-wrap items-start justify-between gap-3 border-b border-gray-100 p-5 dark:border-dark-700">
            <div class="min-w-0">
              <div class="flex flex-wrap items-center gap-2">
                ...
                <span class="rounded px-2 py-0.5 text-xs font-medium" :class="stateClass(plugin.state)">
                  {{ t(`admin.plugins.${plugin.state}`) }}
                </span>
              </div>
              <p class="mt-1 text-xs text-gray-500">
                {{ plugin.plugin_key }}<span v-if="plugin.author"> / {{ plugin.author }}</span>
              </p>
              <p v-if="plugin.description" class="mt-2 text-sm text-gray-600 dark:text-gray-300">
                {{ plugin.description }}
              </p>
            </div>
            <button type="button" class="btn btn-secondary btn-sm" @click="openConfiguration(plugin)">
          ...
          <div class="grid grid-cols-1 gap-x-6 gap-y-4 p-5 md:grid-cols-2">
            <div>
              ...
              <div class="mt-2 flex items-center gap-2">
                <span class="rounded px-2 py-0.5 text-xs font-medium" :class="compatibilityClass(plugin.compatibility.status)">
              ...
              <dl class="mt-3 grid grid-cols-[auto,1fr] gap-x-3 gap-y-1 text-xs">
                <dt class="text-gray-500">{{ t('admin.plugins.currentVersion') }}</dt>
                <dd class="font-mono text-gray-800 dark:text-gray-200">
                  {{ plugin.compatibility.current_sub2api_version }}
                </dd>
                <dt class="text-gray-500">{{ t('admin.plugins.requiredVersion') }}</dt>
                <dd class="font-mono text-gray-800 dark:text-gray-200">
                  {{ plugin.compatibility.required_sub2api_version }}
                </dd>
                <dt class="text-gray-500">{{ t('admin.plugins.recommendedVersion') }}</dt>
                <dd class="font-mono text-gray-800 dark:text-gray-200">
            ...
            <div>
              ...
              <div class="mt-2 flex flex-wrap gap-2 text-xs">
                ...
                >
                  {{ plugin.runtime_healthy ? t('admin.plugins.healthy') : t('admin.plugins.unhealthy') }}
                </span>
                <span class="rounded bg-gray-100 px-2 py-0.5 text-gray-600 dark:bg-dark-700 dark:text-gray-300">
                  {{ t('admin.plugins.signature') }}: {{ t(`admin.plugins.${plugin.signature_status}`) }}
                </span>
              </div>
              <p v-if="plugin.last_error" class="mt-3 break-words text-xs text-red-600 dark:text-red-400">
                {{ plugin.last_error }}
              </p>
              <p v-else-if="plugin.runtime_message" class="mt-3 break-words text-xs text-gray-500">
                {{ plugin.runtime_message }}
              </p>
            </div>

            <div class="md:col-span-2">
              <label class="flex items-center justify-between gap-4 text-xs font-medium text-gray-600 dark:text-gray-300">
          ...
          <div class="flex flex-wrap justify-end gap-2 border-t border-gray-100 px-5 py-4 dark:border-dark-700">
            <button type="button" class="btn btn-secondary btn-sm" :disabled="busyID === plugin.id" @click="testPlugin(plugin)">
            ...
            <button
              ...
              :disabled="busyID === plugin.id || plugin.state === 'starting' || !plugin.compatibility.compatible"
      ...
      <BaseDialog
        ...
        :title="t('admin.plugins.configTitle', { name: configPlugin?.name || '' })"
        width="full"
        @close="closeConfiguration"
      >
        <div class="relative min-h-[520px] overflow-hidden bg-gray-50 dark:bg-dark-900" :style="{ height: `${iframeHeight}px` }">
          <div v-if="uiLoading" class="absolute inset-0 z-10 flex items-center justify-center text-sm text-gray-500">
            {{ t('admin.plugins.loadingUI') }}
          </div>
          <div v-if="uiError" class="absolute inset-0 z-20 flex flex-col items-center justify-center p-8 text-center">
...
async function enablePlugin(plugin: PluginInstallation): Promise<void> {
  ...
  try {
    await pluginStepUp.run(() => adminAPI.plugins.enable(
      plugin.id,
...
async function testPlugin(plugin: PluginInstallation): Promise<void> {
  busyID.value = plugin.id
  try {
    const result = await pluginStepUp.run(() => adminAPI.plugins.test(plugin.id))
    if (result.success) appStore.showSuccess(result.message || t('admin.plugins.testSuccess'))
...
function registerBridgeRequest(requestID: string): void {
  const timeout = window.setTimeout(() => pendingBridgeRequests.delete(requestID), 30_000)
  pendingBridgeRequests.set(requestID, timeout)
}

function postBridgeResult(request: PluginBridgeMessage, payload: Record<string, unknown>): void {
...
async function handleBridgeMessage(event: MessageEvent): Promise<void> {
  ...
  const expectsResponse = ['config.load', 'config.save', 'config.test', 'plugin.status'].includes(message.type || '')
  if (expectsResponse) {
  ...
  try {
    switch (message.type) {
      ...
      case 'config.save': {
        if (!message.config || typeof message.config !== 'object' || Array.isArray(message.config)) {
          throw new Error(t('admin.plugins.bridgeRejected'))
        }
        const config = await pluginStepUp.run(() => adminAPI.plugins.saveConfig(
      ...
      case 'config.test': {
        const result = await pluginStepUp.run(() => adminAPI.plugins.test(configPlugin.value!.id))
        postBridgeResult(message, { ok: result.success, result })
        // The plugin renders successful results; polling must not create host toasts.
        if (!result.success) appStore.showError(result.message || t('common.error'))
        break
      }
      case 'plugin.status': {
        // Read-only status polling needs neither step-up verification nor host toasts.
...
function compatibilityClass(status: PluginInstallation['compatibility']['status']): string {
  if (status === 'compatible') return 'bg-emerald-100 text-emerald-700 dark:bg-emerald-900/30 dark:text-emerald-300'
  if (status === 'untested') return 'bg-amber-100 text-amber-700 dark:bg-amber-900/30 dark:text-amber-300'
