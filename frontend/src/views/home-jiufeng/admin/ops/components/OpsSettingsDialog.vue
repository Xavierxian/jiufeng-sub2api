...
const opsLocalText = (zh: string, en: string) => locale.value.toLowerCase().startsWith('zh') ? zh : en
      ...
      <details class="rounded-2xl bg-gray-50 dark:bg-dark-700/50">
        ...
        <div class="space-y-4 px-4 pb-4">
          ...
          <div class="space-y-3">
            ...
            <div class="flex items-center justify-between">
              <div>
                <label class="text-sm font-medium text-gray-700 dark:text-gray-300">{{ opsLocalText('忽略无效 API Key 错误', 'Ignore invalid API key errors') }}</label>
                <p class="mt-1 text-xs text-gray-500">
                  {{ opsLocalText('忽略由无效、已删除或无法识别的 API Key 引起的错误。', 'Ignore errors caused by invalid, deleted, or unrecognized API keys.') }}
                </p>
              </div>
