    ...
    <TablePageLayout>
      <template #filters>
        <div class="flex flex-wrap items-center gap-3">
          ...
          <button v-if="props.type === 'transfers'" type="button" class="btn btn-primary ml-auto" data-test="affiliate-withdraw-open" @click="withdrawDialog = true">
      ...
      <template #table>
        ...
        >
          ...
          <template #cell-action="{ row }">
            <span :class="['badge whitespace-nowrap', row.action === 'withdraw' ? 'badge-warning' : 'badge-primary']">{{ outflowTypeLabel(row.action) }}</span>
          </template>
...
import AffiliateOfflineWithdrawDialog from '@/views/admin/affiliates/AffiliateOfflineWithdrawDialog.vue'
...
function outflowTypeLabel(action: string | null | undefined): string {
  return action === 'withdraw' ? t('admin.affiliates.outflowTypes.withdraw') : t('admin.affiliates.outflowTypes.transfer')
}
