                  ...
                  <div class="max-h-[70vh] overflow-y-auto p-2">
                    ...
                    <div class="space-y-2 px-3 py-2">
                      <div class="flex items-center justify-between gap-3">
                        <span class="text-sm font-medium text-gray-700 dark:text-gray-200">{{ t('admin.accounts.upstreamBilling.autoProbeSettings') }}</span>
                        <Toggle v-model="upstreamBillingProbeSettings.enabled" :aria-label="t('admin.accounts.upstreamBilling.autoProbeSettings')" />
                      </div>
                      <div class="flex items-center gap-2">
                        <label class="flex-1 text-xs text-gray-500 dark:text-gray-400" for="upstream-billing-probe-interval">{{ t('admin.accounts.upstreamBilling.intervalMinutes') }}</label>
                        <input id="upstream-billing-probe-interval" v-model.number="upstreamBillingProbeSettings.interval_minutes" type="number" min="5" max="1440" class="input h-8 w-20 px-2 text-sm" />
                        <button type="button" class="btn btn-secondary h-8 px-2" :disabled="upstreamBillingSettingsLoading || upstreamBillingSettingsSaving" :title="t('common.save')" @click="saveUpstreamBillingProbeSettings">
        ...
        >
          ...
          <template #cell-name="{ row, value }">
            <div class="flex flex-col">
              <HelpTooltip v-if="accountHomepageUrl(row)" :content="accountHomepageUrl(row)" width-class="w-max max-w-sm break-all" class="-ml-1 self-start">
                <template #trigger>
                  <a :href="accountHomepageUrl(row)" target="_blank" rel="noopener noreferrer" class="border-b border-dotted border-gray-300 font-medium text-gray-900 dark:border-gray-600 dark:text-white">{{ value }}</a>
                </template>
              </HelpTooltip>
          ...
          <template #cell-upstream_billing_rate="{ row }">
            <UpstreamBillingRateCell :account="row" :interval-minutes="upstreamBillingProbeSettings.interval_minutes" :now="upstreamBillingNow" :probing="probingUpstreamBilling.has(row.id)" @probe="handleProbeUpstreamBilling(row)" />
          </template>
...
import TablePageLayout from '@/views/home-jiufeng/JFTablePageLayout.vue'
import DataTable from '@/views/home-jiufeng/JFDataTable.vue'
...
const upstreamBillingProbeSettings = reactive<UpstreamBillingProbeSettings>({ enabled: true, interval_minutes: 30 })
...
const saveUpstreamBillingProbeSettings = async () => {
  ...
  try {
    Object.assign(upstreamBillingProbeSettings, await adminAPI.accounts.updateUpstreamBillingProbeSettings({ ...upstreamBillingProbeSettings }))
...
const handleBulkProbeUpstreamBilling = async () => {
  ...
  if (accountIDs.length === 0) return appStore.showError(t('admin.accounts.upstreamBilling.noEligibleAccounts'))
  if (accountIDs.length > 20) return appStore.showError(t('admin.accounts.upstreamBilling.batchLimit'))
  ...
  try {
    ...
    results.forEach(result => { if (result.snapshot) patchUpstreamBillingSnapshot(result.account_id, result.snapshot) })
    const failed = results.filter(result => result.error).length
    if (failed > 0) appStore.showError(t('admin.accounts.upstreamBilling.batchPartial', { success: results.length - failed, failed }))
    else appStore.showSuccess(t('admin.accounts.upstreamBilling.batchCompleted', { count: results.length }))
  } catch (error) {
...
const patchUpstreamBillingSnapshot = (accountID: number, snapshot: UpstreamBillingProbeSnapshot) => {
  ...
  if (!account) return
  patchAccountInList({ ...account, extra: { ...account.extra, upstream_billing_probe: snapshot } })
}
