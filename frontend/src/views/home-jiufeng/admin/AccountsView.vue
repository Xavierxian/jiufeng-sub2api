...
const handleManualRefresh = async () => {
  await Promise.all([load(), loadUpstreamBillingProbeSettings()])
...
const loadUpstreamBillingProbeSettings = async () => {
  ...
  try {
    ...
    Object.assign(upstreamBillingProbeSettings, settings)
...
const saveUpstreamBillingProbeSettings = async () => {
  ...
  try {
    const settings = await adminAPI.accounts.updateUpstreamBillingProbeSettings({ ...upstreamBillingProbeSettings })
    Object.assign(upstreamBillingProbeSettings, settings)
    upstreamBillingProbeGloballyEnabled.value = settings.enabled
