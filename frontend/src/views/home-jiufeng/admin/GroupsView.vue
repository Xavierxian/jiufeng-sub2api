  ...
  <AppLayout>
      ...
      >
        ...
        >
          ...
          <label class="flex items-start gap-2">
            <input v-model="createForm.force_openai_fast" type="checkbox" class="mt-0.5" />
            <span>
              <span class="block text-sm text-gray-700 dark:text-gray-300">{{ t("admin.groups.openaiFast.force") }}</span>
              <span class="block text-xs text-gray-500 dark:text-gray-400">{{ t("admin.groups.openaiFast.hint") }}</span>
            </span>
          </label>
          <label class="mt-3 flex items-start gap-2">
            <input v-model="createForm.free_openai_fast" type="checkbox" class="mt-0.5" />
            <span>
              <span class="block text-sm text-gray-700 dark:text-gray-300">{{ t("admin.groups.openaiFast.free") }}</span>
              <span class="block text-xs text-gray-500 dark:text-gray-400">{{ t("admin.groups.openaiFast.freeHint") }}</span>
            </span>
          </label>
        </div>

        <!-- Codex Live is available to OpenAI and Composite groups. -->
        <div
    ...
    >
      ...
      >
        ...
        <!-- Fixed accounts for fetching the Codex model manifest (OpenAI edit only). -->
        ...
        >
          ...
          <label class="flex items-start gap-2">
            <input v-model="editForm.force_openai_fast" type="checkbox" class="mt-0.5" />
            <span>
              <span class="block text-sm text-gray-700 dark:text-gray-300">{{ t("admin.groups.openaiFast.force") }}</span>
              <span class="block text-xs text-gray-500 dark:text-gray-400">{{ t("admin.groups.openaiFast.hint") }}</span>
            </span>
          ...
          <label class="mt-3 flex items-start gap-2">
            <input v-model="editForm.free_openai_fast" type="checkbox" class="mt-0.5" />
            <span>
              <span class="block text-sm text-gray-700 dark:text-gray-300">{{ t("admin.groups.openaiFast.free") }}</span>
              <span class="block text-xs text-gray-500 dark:text-gray-400">{{ t("admin.groups.openaiFast.freeHint") }}</span>
            </span>
...
} from "@/views/admin/groupsOpenAIFast";
import {
...
const submitCreateAllowlistCustomEntry = () => {
  const error = addCustomModelsListItem(
...
const handleCreateGroup = async () => {
  ...
  if (!authStore.isSimpleMode && !validateProfitControlForm(createForm)) {
    return;
  }
  if (
...
const handleEdit = async (group: AdminGroup) => {
  ...
  for (const id of editCodexManifestConfig.account_ids) {
    adminAPI.accounts
      ...
      .catch(() => {
        // The field falls back to #<id> when a saved account can no longer be resolved.
      });
  }
  // 加载模型路由规则（异步加载账号名称）
...
const handleUpdateGroup = async () => {
  ...
  if (!authStore.isSimpleMode && !validateProfitControlForm(editForm)) {
    return;
  }
  if (
    !authStore.isSimpleMode &&
