    ...
    <div class="space-y-6">
      ...
      <div class="card p-6">
        ...
        <div class="mt-4 flex flex-wrap gap-2">
          ...
          <button type="button" class="btn btn-primary btn-sm" :disabled="savingImageStorage" @click="saveImageStorageConfig">{{ savingImageStorage ? t('common.loading') : t('common.save') }}</button>
        </div>
      </div>
...
async function testImageStorage() {
  ...
  try {
    ...
    if (result.ok) appStore.showSuccess(result.message || t('admin.backup.s3.testSuccess'))
    else appStore.showError(result.message || t('admin.backup.s3.testFailed'))
  } catch (error) {
    appStore.showError((error as { message?: string })?.message || t('errors.networkError'))
  } finally {
