...
const scheduleValidationError = computed(() => {
  ...
  if (!counts.every(value => Number.isSafeInteger(value) && value >= 0)) return t('admin.backup.archive.invalidRetention')
  if (archiveForm.value.enabled && !archiveForm.value.days.length && !archiveForm.value.include_month_end) return t('admin.backup.archive.selectDates')
  return ''
})
const retentionPreview = computed(() => {
  ...
  const dates = [...archiveForm.value.days].sort((a, b) => a - b).map(day => t('admin.backup.archive.day', { day })).concat(
