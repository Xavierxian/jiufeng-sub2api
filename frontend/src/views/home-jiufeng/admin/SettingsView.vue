            ...
            <div class="space-y-5 p-6">
              ...
              <div v-if="form.api_key_acl_trust_forwarded_ip" class="border-t border-gray-100 pt-4 dark:border-dark-700">
                ...
                <div class="mt-3 rounded-lg border border-gray-300 bg-white p-2 dark:border-dark-500 dark:bg-dark-700">
                  <div class="flex flex-wrap items-center gap-2">
                    <span v-for="header in form.forwarded_client_ip_headers" :key="header" class="inline-flex items-center gap-1 rounded bg-gray-100 px-2 py-1 text-xs font-mono text-gray-700 dark:bg-dark-600 dark:text-gray-200">
                      <span>{{ header }}</span>
                      <button type="button" class="rounded-full text-gray-500 hover:bg-gray-200 hover:text-gray-700 dark:text-gray-300 dark:hover:bg-dark-500 dark:hover:text-white" :aria-label="t('admin.settings.apiKeyAcl.removeForwardedClientIpHeader', { header })" @click="removeForwardedClientIpHeader(header)">
                        <Icon name="x" size="xs" class="h-3.5 w-3.5" :stroke-width="2" />
                      </button>
                    </span>
                    <input id="forwarded-client-ip-headers" v-model="forwardedClientIpHeaderDraft" type="text" class="min-w-[220px] flex-1 bg-transparent px-2 py-1 text-sm font-mono text-gray-900 outline-none placeholder:text-gray-400 dark:text-white dark:placeholder:text-gray-500" :placeholder="t('admin.settings.apiKeyAcl.forwardedClientIpHeadersPlaceholder')" @keydown="handleForwardedClientIpHeaderKeydown" @blur="commitForwardedClientIpHeaderDraft" @paste="handleForwardedClientIpHeaderPaste" />
                  </div>
                </div>
                <p class="mt-2 text-xs text-gray-500 dark:text-gray-400">
        ...
        <div v-show="activeTab === 'gateway'" class="space-y-6">
          ...
          <div class="card">
            ...
            <div class="space-y-5 p-6">
              ...
              <div v-if="!form.openai_advanced_scheduler_enabled" class="flex items-center justify-between border-t border-gray-100 pt-5 dark:border-dark-700">
                <div>
                  <label class="text-sm font-medium text-gray-700 dark:text-gray-300">{{ t("admin.settings.openaiExperimentalScheduler.lowRatePriorityTitle") }}</label>
                  <p class="mt-0.5 text-xs text-gray-500 dark:text-gray-400">{{ t("admin.settings.openaiExperimentalScheduler.lowRatePriorityDescription") }}</p>
                </div>
                <Toggle v-model="form.openai_low_upstream_rate_priority_enabled" data-testid="openai-low-rate-priority-toggle" />
              </div>

              <div v-if="!form.openai_advanced_scheduler_enabled && form.openai_low_upstream_rate_priority_enabled" class="flex flex-col items-stretch gap-3 border-t border-gray-100 pt-5 sm:flex-row sm:items-start sm:justify-between sm:gap-6 dark:border-dark-700">
                <div class="min-w-0">
                  <label class="text-sm font-medium text-gray-700 dark:text-gray-300" for="openai-oauth-scheduling-rate-multiplier">{{ t("admin.settings.openaiExperimentalScheduler.oauthRateTitle") }}</label>
                  <p class="mt-0.5 text-xs text-gray-500 dark:text-gray-400">{{ t("admin.settings.openaiExperimentalScheduler.oauthRatePriorityDescription") }}</p>
                </div>
                <input id="openai-oauth-scheduling-rate-multiplier" v-model.number="form.openai_oauth_scheduling_rate_multiplier" class="input w-full sm:w-32" min="0" step="0.01" type="number" />
              </div>
              ...
              <div v-if="form.openai_advanced_scheduler_enabled" class="flex flex-col items-stretch gap-3 border-t border-gray-100 pt-5 sm:flex-row sm:items-start sm:justify-between sm:gap-6 dark:border-dark-700">
                <div class="min-w-0">
                  <label class="text-sm font-medium text-gray-700 dark:text-gray-300" for="openai-oauth-scheduling-rate-multiplier-weighted">{{ t("admin.settings.openaiExperimentalScheduler.oauthRateTitle") }}</label>
                  <p class="mt-0.5 text-xs text-gray-500 dark:text-gray-400">{{ t("admin.settings.openaiExperimentalScheduler.oauthRateWeightedDescription") }}</p>
                </div>
                <input id="openai-oauth-scheduling-rate-multiplier-weighted" v-model.number="form.openai_oauth_scheduling_rate_multiplier" class="input w-full sm:w-32" min="0" step="0.01" type="number" />
              </div>
        ...
        <div class="card">
          ...
          <div class="space-y-5 p-6">
            ...
            <div v-if="form.affiliate_enabled" class="space-y-6">
              <div class="flex items-center justify-between">
                <div>
                  <label class="text-sm font-medium text-gray-700 dark:text-gray-300">{{ t('admin.settings.features.affiliate.adminRechargeRebate') }}</label>
                  <p class="mt-0.5 text-xs text-gray-500 dark:text-gray-400">{{ t('admin.settings.features.affiliate.adminRechargeRebateHint') }}</p>
                </div>
...
import BackupSettings from "@/views/home-jiufeng/admin/BackupView.vue";
import EmailTemplateEditor from "@/views/home-jiufeng/admin/settings/EmailTemplateEditor.vue";
import OpenAIFastPolicyUserSelector from "@/views/home-jiufeng/admin/settings/OpenAIFastPolicyUserSelector.vue";
...
function normalizeForwardedClientIpHeader(raw: string): string {
  ...
  return header.toLowerCase().split("-").map((part) => `${part.charAt(0).toUpperCase()}${part.slice(1)}`).join("-");
}
...
function removeForwardedClientIpHeader(header: string) {
  form.forwarded_client_ip_headers = form.forwarded_client_ip_headers.filter((item) => item !== header);
}

function addForwardedClientIpHeader(raw: string): "added" | "duplicate" | "invalid" | "full" {
  const header = normalizeForwardedClientIpHeader(raw);
  if (!header) return "invalid";
  if (form.forwarded_client_ip_headers.some((item) => item.toLowerCase() === header.toLowerCase())) return "duplicate";
  if (form.forwarded_client_ip_headers.length >= maxForwardedClientIpHeaders) return "full";
  form.forwarded_client_ip_headers = [...form.forwarded_client_ip_headers, header];
  return "added";
}

function showForwardedClientIpHeaderError(result: "added" | "duplicate" | "invalid" | "full") {
  if (result === "invalid") appStore.showError(t("admin.settings.apiKeyAcl.forwardedClientIpHeaderInvalid"));
  if (result === "full") appStore.showError(t("admin.settings.apiKeyAcl.forwardedClientIpHeadersLimit", { max: maxForwardedClientIpHeaders }));
}

function commitForwardedClientIpHeaderDraft() {
  if (!forwardedClientIpHeaderDraft.value) return;
  showForwardedClientIpHeaderError(addForwardedClientIpHeader(forwardedClientIpHeaderDraft.value));
...
function handleForwardedClientIpHeaderKeydown(event: KeyboardEvent) {
  ...
  } else if (event.key === "Backspace" && !forwardedClientIpHeaderDraft.value && form.forwarded_client_ip_headers.length > 0) {
...
function handleForwardedClientIpHeaderPaste(event: ClipboardEvent) {
  ...
  for (const token of text.split(/[,\r\n]+/)) {
    if (token.trim()) showForwardedClientIpHeaderError(addForwardedClientIpHeader(token));
  }
}
...
async function saveSettings() {
  ...
  try {
    ...
    const payload: UpdateSettingsRequest = {
      ...
      forwarded_client_ip_headers: normalizeForwardedClientIpHeaders(
