          ...
          <div class="card">
            ...
            <div class="space-y-5 p-6">
              <div class="flex items-center justify-between">
                ...
                <Toggle v-model="captchaMasterEnabled" data-testid="captcha-enabled-toggle" />
              </div>
              ...
              >
                <div>
                  ...
                  <div class="grid grid-cols-1 gap-2 rounded-lg bg-gray-100 p-1 sm:grid-cols-3 dark:bg-dark-700">
                    <button
                      v-for="provider in (['turnstile', 'tencent', 'aliyun'] as const)"
                      :key="provider"
                      type="button"
                      :data-testid="`captcha-provider-${provider}`"
                      class="rounded-md px-3 py-2 text-sm font-medium transition"
                      :class="captchaProviderSelection === provider
                        ? 'bg-white text-primary-700 shadow-sm dark:bg-dark-800 dark:text-primary-300'
                        : 'text-gray-600 hover:text-gray-900 dark:text-dark-300 dark:hover:text-white'"
                      @click="selectCaptchaProvider(provider)"
                    >
                      {{ t(`admin.settings.captcha.provider${provider === 'turnstile' ? 'Turnstile' : provider === 'tencent' ? 'Tencent' : 'Aliyun'}`) }}
                    </button>
                  </div>
                </div>

                <div v-if="captchaProviderSelection === 'turnstile'" class="grid grid-cols-1 gap-6">
                  <div>
                    <label
                      class="mb-2 block text-sm font-medium text-gray-700 dark:text-gray-300"
                    >
                ...
                <div
                  v-else-if="captchaProviderSelection === 'tencent'"
                ...
                >
                  <div class="max-w-sm md:col-span-2">
                    ...
                    <div class="grid grid-cols-2 gap-2 rounded-lg bg-gray-100 p-1 dark:bg-dark-700">
                      <button
                        ...
                        :class="form.tencent_captcha_region !== 'intl' ? 'bg-white text-primary-700 shadow-sm dark:bg-dark-800 dark:text-primary-300' : 'text-gray-600 hover:text-gray-900 dark:text-dark-300 dark:hover:text-white'"
                  ...
                  <div>
                    ...
                    <input v-model="form.tencent_captcha_app_id" type="text" inputmode="numeric" class="input font-mono text-sm" placeholder="123456789" />
                  </div>
                  <div>
                    <label class="mb-2 block text-sm font-medium text-gray-700 dark:text-gray-300">
                  ...
                  <div class="md:col-span-2">
                    ...
                    <div class="mt-3 flex flex-wrap gap-x-4 gap-y-2 text-sm">
                      <a :href="tencentCaptchaLinks.console" target="_blank" rel="noopener noreferrer" class="text-primary-600 hover:text-primary-500">
                        {{ t("admin.settings.tencentCaptcha.openCaptchaConsole") }}
                      </a>
                      <a :href="tencentCaptchaLinks.cloudKeys" target="_blank" rel="noopener noreferrer" class="text-primary-600 hover:text-primary-500">
                        {{ t("admin.settings.tencentCaptcha.createCloudKeys") }}
                      </a>
                      <a :href="tencentCaptchaLinks.webDocs" target="_blank" rel="noopener noreferrer" class="text-primary-600 hover:text-primary-500">
                ...
                <div v-else class="grid grid-cols-1 gap-6">
                  <div class="grid grid-cols-1 gap-6 sm:grid-cols-2">
                    <div>
                      ...
                      <div class="grid grid-cols-2 gap-2 rounded-lg bg-gray-100 p-1 dark:bg-dark-700">
                        <button
                          type="button"
                          class="rounded-md px-3 py-1.5 text-sm font-medium transition"
                          :class="form.aliyun_captcha_region !== 'sgp' ? 'bg-white text-primary-700 shadow-sm dark:bg-dark-800 dark:text-primary-300' : 'text-gray-600 dark:text-dark-300'"
                    ...
                    <div>
                      ...
                      <input v-model="form.aliyun_captcha_prefix" type="text" class="input font-mono text-sm" placeholder="14xxxxx" />
                      <p class="mt-1.5 text-xs text-gray-500 dark:text-gray-400">
                  ...
                  <div>
                    ...
                    <input v-model="form.aliyun_captcha_scene_id" type="text" class="input font-mono text-sm" placeholder="1cxxxxxx" />
                    <p class="mt-1.5 text-xs text-gray-500 dark:text-gray-400">
                  ...
                  <div>
                    ...
                    <input v-model="form.aliyun_captcha_access_key_id" type="text" class="input font-mono text-sm" placeholder="LTAI..." />
                    <p class="mt-1.5 text-xs text-gray-500 dark:text-gray-400">
                  ...
                  <div>
                    ...
                    <input
                      ...
                      autocomplete="new-password"
                      class="input font-mono text-sm"
                      :placeholder="form.aliyun_captcha_access_key_secret_configured ? t('admin.settings.tencentCaptcha.keepExisting') : ''"
                    />
                    <p class="mt-1.5 text-xs text-gray-500 dark:text-gray-400">
                      {{ form.aliyun_captcha_access_key_secret_configured ? t("admin.settings.aliyunCaptcha.accessKeySecretConfiguredHint") : t("admin.settings.aliyunCaptcha.accessKeySecretHint") }}
                    </p>
                  </div>
                </div>
              </div>
...
import PanelRateLimitCard from "@/views/home-jiufeng/admin/settings/PanelRateLimitCard.vue";
...
async function saveSettings() {
  ...
  try {
    ...
    const payload: UpdateSettingsRequest = {
      ...
      // ?????? v-model.number ???????? int ??????? 400 ???????
      // ?/??????? 180???? parseAuditLogRetentionDays("") ?????0 ?????????
