<template>
  <AppLayout>
    <div class="jf-responsive-page mx-auto max-w-6xl space-y-6">
      <!-- Loading State -->
      ...
      <form v-else @submit.prevent="saveSettings" class="space-y-6" novalidate>
        ...
        <div v-show="activeTab === 'gateway'" class="space-y-6">
          ...
          <div class="card" data-testid="opencode-go-usage-global-settings">
            <div class="border-b border-gray-100 px-6 py-4 dark:border-dark-700">
              <h2 class="text-lg font-semibold text-gray-900 dark:text-white">{{ t('admin.settings.opencodeGoUsage.title') }}</h2>
              <p class="mt-1 text-sm text-gray-500 dark:text-gray-400">{{ t('admin.settings.opencodeGoUsage.description') }}</p>
            </div>
            <div class="space-y-5 p-6">
              <div v-if="opencodeGoUsageLoading" class="text-sm text-gray-500">{{ t('common.loading') }}</div>
              <template v-else>
                <div class="flex items-center justify-between gap-4">
                  <div>
                    <label class="font-medium text-gray-900 dark:text-white">{{ t('admin.settings.opencodeGoUsage.enabled') }}</label>
                    <p class="text-sm text-gray-500 dark:text-gray-400">{{ t('admin.settings.opencodeGoUsage.enabledHint') }}</p>
                  </div>
                  <Toggle v-model="opencodeGoUsageForm.enabled" data-testid="opencode-go-usage-global-enabled" />
                </div>
                <div v-if="opencodeGoUsageForm.enabled" class="grid gap-4 border-t border-gray-100 pt-4 sm:grid-cols-2 dark:border-dark-700">
                  <label class="text-sm font-medium text-gray-700 dark:text-gray-300">
                    {{ t('admin.settings.opencodeGoUsage.debounceMinutes') }}
                    <input v-model.number="opencodeGoUsageForm.debounce_minutes" type="number" min="1" max="60" class="input mt-2" />
                    <span class="mt-1 block text-xs font-normal text-gray-500">{{ t('admin.settings.opencodeGoUsage.debounceHint') }}</span>
                  </label>
                  <label class="text-sm font-medium text-gray-700 dark:text-gray-300">
                    {{ t('admin.settings.opencodeGoUsage.intervalMinutes') }}
                    <input v-model.number="opencodeGoUsageForm.interval_minutes" type="number" min="5" max="1440" class="input mt-2" />
                    <span class="mt-1 block text-xs font-normal text-gray-500">{{ t('admin.settings.opencodeGoUsage.intervalHint') }}</span>
                  </label>
                </div>
                <div class="flex justify-end border-t border-gray-100 pt-4 dark:border-dark-700">
            ...
            <div class="space-y-5 p-6">
              <div class="grid gap-5 border-b border-gray-100 pb-5 dark:border-dark-700 md:grid-cols-[minmax(0,1fr)_auto] md:items-end">
                ...
                <div class="border-b border-gray-100 pb-5 dark:border-dark-700 md:col-span-2">
                  <label for="openai-ttft-mode" class="text-sm font-medium text-gray-700 dark:text-gray-300">
                    {{ t("admin.settings.gatewayForwarding.openaiTTFTMode") }}
                  </label>
                  <select id="openai-ttft-mode" v-model="form.openai_ttft_mode" class="input mt-2 w-full" data-testid="openai-ttft-mode">
              ...
              <!-- Claude Code 客户端版本号 -->
              <div>
                <label class="mb-2 block text-sm font-medium text-gray-700 dark:text-gray-300">{{ t('admin.settings.gatewayForwarding.claudeCodeClientVersion') }}</label>
                <input v-model="form.claude_code_client_version" type="text" class="input w-full font-mono text-sm" placeholder="2.1.280" />
                <p class="mt-1.5 text-xs text-gray-500 dark:text-gray-400">{{ t('admin.settings.gatewayForwarding.claudeCodeClientVersionHint') }}</p>
              </div>
              <div class="flex items-center justify-between">
                <div>
                  <label class="text-sm font-medium text-gray-700 dark:text-gray-300">{{ t('admin.settings.gatewayForwarding.claudeCodeVersionAutoSync') }}</label>
                  <p class="mt-0.5 text-xs text-gray-500 dark:text-gray-400">{{ t('admin.settings.gatewayForwarding.claudeCodeVersionAutoSyncHint') }}</p>
                  <p v-if="claudeSyncedVersionLabel" class="mt-0.5 text-xs text-gray-500 dark:text-gray-400">{{ claudeSyncedVersionLabel }}</p>
                </div>
...
const opencodeGoUsageForm = reactive({ enabled: false, interval_minutes: 15, debounce_minutes: 1 });
...
const form = reactive<SettingsForm>({
  ...
  openai_oauth_scheduling_rate_multiplier: null as number | null,
...
async function saveSettings() {
  ...
  try {
    ...
    const oauthSchedulingRateValue = typeof oauthSchedulingRate === "number" ? oauthSchedulingRate : null;
    if (oauthSchedulingRateValue !== null && (!Number.isFinite(oauthSchedulingRateValue) || oauthSchedulingRateValue < 0)) {
...
async function loadOpenCodeGoUsageSettings() {
  ...
  try {
    ...
    // Keep disabled defaults when this optional setting cannot be loaded.
  } finally {
...
@media (min-width: 768px) {
  ...
  .settings-tab {
    @apply min-w-max flex-none basis-auto overflow-visible px-3 text-[13px];
  }
...
.settings-tab.settings-tab-active::before {
  opacity: 0;
}
...
.settings-tab-label {
  @apply min-w-max overflow-visible whitespace-nowrap leading-none;
}
</style>
