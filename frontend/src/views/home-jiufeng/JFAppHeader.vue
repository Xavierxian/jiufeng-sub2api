  ...
  <header class="jf-cloud-header">
    <div class="jf-cloud-header__inner">
      <div class="jf-cloud-header__left">
        ...
        <div
          v-if="authStore.isAdmin && displayVersion"
          ref="versionRef"
        ...
        <div
          class="jf-cloud-region-notice"
          role="note"
          tabindex="0"
          :aria-label="serviceRegionNotice"
        >
          <div class="jf-cloud-region-notice__viewport" aria-hidden="true">
            <div class="jf-cloud-region-notice__track">
              <span
                v-for="copy in 2"
                :key="copy"
                class="jf-cloud-region-notice__item"
              >
                {{ serviceRegionNotice }}
              </span>
            </div>
      ...
      <div class="jf-cloud-header__right">
        ...
        <router-link
          ...
          class="jf-cloud-icon-link jf-cloud-model-plaza"
        >
        ...
        <div v-if="user && subscriptionFeatureEnabled" class="jf-cloud-subscription-mini">
          <SubscriptionProgressMini />
        </div>

        <div
          v-if="user"
          class="jf-cloud-balance group relative"
        >
          <svg
            class="h-4 w-4"
            fill="none"
          ...
          <div
            class="pointer-events-none absolute right-0 top-full z-50 mt-2 hidden w-56 rounded-lg border border-gray-200 bg-white p-3 text-xs text-gray-700 shadow-lg group-hover:block dark:border-dark-700 dark:bg-dark-800 dark:text-gray-200"
          >
            <div class="flex items-center justify-between gap-3">
              <span class="font-normal text-gray-500 dark:text-dark-400">{{ balanceAvailableText }}</span>
              <span class="font-medium text-gray-900 dark:text-white">{{ formatHeaderMoney(availableBalance) }}</span>
            </div>
            <div class="mt-2 flex items-center justify-between gap-3">
              <span class="font-normal text-gray-500 dark:text-dark-400">{{ balanceFrozenText }}</span>
            ...
            <div class="mt-2 border-t border-gray-100 pt-2 dark:border-dark-700">
              <div class="flex items-center justify-between gap-3">
                <span class="font-normal text-gray-500 dark:text-dark-400">{{ balanceTotalText }}</span>
            ...
            <div v-if="dropdownOpen" class="dropdown right-0 mt-2 w-56">
              ...
              <div class="py-1">
                ...
                <a
                  ...
                  href="https://github.com/Xavierxian/jiufeng-sub2api"
                  target="_blank"
                  rel="noopener noreferrer"
                  @click="closeDropdown"
                  class="dropdown-item"
...
const serviceRegionNotice = computed(() =>
  locale.value === 'zh'
...
const releaseUrl = computed(() => {
  ...
  return url && url !== '#' ? url : 'https://github.com/Xavierxian/jiufeng-sub2api/releases'
})
