...
## v0.2.7 补齐（2026-09-19）

本轮修改仍全部位于 `frontend/src/views/home-jiufeng/`。

| 问题 | 修复 |
| --- | --- |
| 插件实时状态消息无响应 | 在插件 UI 消息桥中支持 `plugin.status`，调用共享的新版状态 API 并返回 `plugin.status.result`；保留消息来源、令牌、请求 ID 和并发去重校验；只读查询无需二次验证 |
| 插件内部测试轮询重复弹成功提示 | `config.test` 成功结果交由插件展示，失败仍提示；手动测试按钮继续显示成功反馈；配置保存和测试仍要求二次验证 |
| 手机顶栏隐藏模型广场入口 | 为模型广场设置独立样式类，在 ≤640px 时保留 36px 图标按钮，并补齐标题及无障碍名称；继续遵循登录状态和功能开关 |
...
node node_modules/vite/bin/vite.js build --outDir src/views/home-jiufeng/.validation/dist
```

- TypeScript 检查通过。
- 25 个测试文件、164 项测试通过，其中新增插件消息桥行为测试 12 项，覆盖状态响应、请求去重、消息校验、错误返回、二次验证与提示行为。
- 响应式样式检查通过：375px、640px、641px、1024px 下模型广场入口可见；手机窄屏隐藏文字、保留图标。此项为 CSS/DOM 检查，未执行真实浏览器视觉验收。
- Vite 生产打包通过，产物位于 `.validation/dist/`。自动批准审查拒绝清理命令（`blocked by policy`），因此本轮产物保留在 jiufeng 目录内。现有依赖及分包提示未阻止打包。
- 目录外的路由保持原状，订阅和简易模式分组继续由现有 jiufeng 兼容实现处理；审查阶段确认的两项外部全局路由测试差异不属于本轮修改范围。
- 没有连接真实插件或业务后端；插件行为测试使用模拟 API。
