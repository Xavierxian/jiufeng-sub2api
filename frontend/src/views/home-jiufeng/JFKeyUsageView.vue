<template>
  <div class="jf-key-usage-page relative flex min-h-screen flex-col">
    <!-- Header (same pattern as HomeView) -->
    <header class="jf-key-usage-header relative z-20 px-6 py-3">
      <nav class="mx-auto flex max-w-6xl items-center justify-between">
        <router-link to="/home" class="flex items-center gap-3">
          <div class="jf-key-usage-logo h-10 w-10 overflow-hidden">
      ...
      <div class="max-w-xl mx-auto mb-14">
        <div class="flex flex-col gap-3 sm:flex-row">
          ...
          <button
            @click="queryKey"
            :disabled="isQuerying"
            class="flex h-12 items-center justify-center gap-2 whitespace-nowrap rounded-lg bg-primary-500 px-7 text-sm font-medium text-white transition-all hover:bg-primary-600 active:scale-[0.97] disabled:opacity-60"
          >
...
.jf-key-usage-page :deep(.rounded-2xl) {
  border-radius: 8px !important;
}

.jf-key-usage-page :deep(.rounded-xl) {
  border-radius: 6px !important;
}

:global(.dark .jf-key-usage-page) {
  background: #0b0b0d;
  color: #f3f4f6;
}

:global(.dark .jf-key-usage-header) {
  border-color: #2a2a30;
  background: rgba(18, 18, 20, 0.95);
}

:global(.dark .jf-key-usage-logo) {
  border-color: rgba(32, 199, 179, 0.24);
  background: rgba(32, 199, 179, 0.14);
}

@media (max-width: 640px) {
  .jf-key-usage-page main {
    padding: 32px 14px;
  }

  .jf-key-usage-page footer {
    padding-inline: 14px;
  }
}
</style>
