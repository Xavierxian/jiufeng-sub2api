    ...
    <main>
      <section class="sx-hero">
        <div class="sx-intro-layer" aria-hidden="true">
          <div class="sx-intro-curtain">
            <i />
            <i />
          </div>

          <div class="sx-intro-topline">
          ...
          <div class="sx-intro-rail sx-intro-rail-left">
            <p>ROUTING / 01</p>
            <div class="sx-intro-rail-item">
              <span>01</span>
          ...
          <div class="sx-intro-rail sx-intro-rail-right">
            <p>UPSTREAM / 08</p>
            <div class="sx-intro-rail-item">
              <span>LATENCY</span>
              <strong>12<span>MS</span></strong>
              <em>EDGE RESPONSE</em>
              <i />
            </div>
            <div class="sx-intro-rail-item">
            ...
            <div class="sx-intro-rail-item">
              <span>RETENTION</span>
              <strong>ZERO</strong>
              <em>REQUEST MEMORY</em>
              <i />
            </div>
          </div>

          <div class="sx-intro-center">
            <p class="sx-intro-overline">SECURE MODEL ROUTING / LIVE</p>
            <div class="sx-intro-sequence">
              <span v-for="(step, index) in introSequence" :key="step" :class="{ 'is-active': index === 0 }">
                <b>0{{ index + 1 }}</b>{{ step }}
              </span>
            </div>
            <div class="sx-intro-network">
              <span class="sx-intro-axis sx-intro-axis-y" />
              <span class="sx-intro-network-ring sx-intro-network-ring-inner" />
              <span class="sx-intro-network-ring sx-intro-network-ring-outer" />
              <span
                v-for="route in introRoutes"
                :key="route.angle"
                class="sx-intro-route"
                :style="{
                  '--sx-route-angle': `${route.angle}deg`,
                  '--sx-route-length': `${route.length}%`,
                  '--sx-route-delay': `${route.delay}s`,
                  '--sx-route-color': route.color,
                }"
              />
              <span
                v-for="node in introNodes"
                :key="node.label"
                class="sx-intro-node"
                :style="{
                  '--sx-node-x': `${node.x}%`,
                  '--sx-node-y': `${node.y}%`,
                  '--sx-node-delay': `${node.delay}s`,
                  '--sx-node-color': node.color,
                }"
              >
                <span class="sx-intro-node-mark">
                  <svg
                    :viewBox="modelIcons[node.icon].viewBox"
                    aria-hidden="true"
                    fill="currentColor"
                    fill-rule="evenodd"
                  ...
                  >
                    <path
                      v-for="path in modelIcons[node.icon].paths"
                      :key="path.d"
                      :d="path.d"
              ...
              <div class="sx-intro-core">
                <span>{{ homeCopy.brandMark }}</span>
                <small>ROUTING CORE</small>
                <b>ONLINE</b>
              </div>
            </div>
            <div class="sx-intro-readout">
              <div><span>MODEL NODES</span><strong>08</strong></div>
              <div><span>RELAY MODE</span><strong>SECURE</strong></div>
              <div><span>UPSTREAM</span><strong>READY</strong></div>
            </div>
          </div>

          <div class="sx-intro-footer">
            <div class="sx-intro-phase-label">
        ...
        <div class="sx-routing-halo" aria-hidden="true">
          <span class="sx-routing-halo-field is-wide" />
          <span class="sx-routing-halo-field is-tall" />
          <span class="sx-routing-halo-field is-slice" />
          <span class="sx-routing-halo-sweep is-upper" />
          <span class="sx-routing-halo-sweep is-lower" />
          <span class="sx-routing-halo-signal"><i /></span>
          <span
            v-for="route in haloRoutes"
            :key="route.angle"
            class="sx-routing-halo-route"
            :style="{
              '--sx-halo-route-angle': `${route.angle}deg`,
              '--sx-halo-route-color': route.color,
              '--sx-halo-route-delay': `${route.delay}s`,
            }"
          />
          <span
            v-for="node in haloNodes"
            :key="node.label"
            class="sx-routing-halo-node"
            :style="{
              '--sx-halo-node-x': `${node.x}%`,
              '--sx-halo-node-y': `${node.y}%`,
              '--sx-halo-node-color': node.color,
              '--sx-halo-node-delay': `${node.delay}s`,
            }"
          >
            <b>
              <svg
          ...
          <span class="sx-routing-halo-core">
            <b>{{ homeCopy.brandMark }}</b>
            <small>ROUTING</small>
          </span>
        </div>
                  ...
                  <template v-if="currentWhyCard.index === '01'">
                    ...
                    <div class="sx-model-map">
                      <svg class="sx-model-lines" viewBox="0 0 430 190" focusable="false" aria-hidden="true">
                        <path d="M60 32 C 140 32, 164 95, 220 95" />
                        <path d="M60 95 L 220 95" />
                        <path d="M60 158 C 140 158, 164 95, 220 95" />
                        <path d="M282 95 L 376 95" />
                        <circle class="sx-flow-dot" r="4">
                          <animateMotion dur="2.9s" repeatCount="indefinite" path="M60 32 C 140 32, 164 95, 220 95" />
                        </circle>
                        <circle class="sx-flow-dot" r="4">
                          <animateMotion dur="2.5s" begin=".35s" repeatCount="indefinite" path="M60 95 L 220 95" />
                        </circle>
                        <circle class="sx-flow-dot" r="4">
                          <animateMotion dur="3.1s" begin=".7s" repeatCount="indefinite" path="M60 158 C 140 158, 164 95, 220 95" />
                        </circle>
                        <circle class="sx-flow-dot is-out" r="4">
                          <animateMotion dur="1.9s" begin=".2s" repeatCount="indefinite" path="M282 95 L 376 95" />
                        </circle>
                      </svg>
...
<span v-for="(line, index) in terminalLines" :key="`${line.text}-${index}`" class="sx-term-line" :class="`is-${line.kind}`">{{ line.text }}</span><span v-if="currentTerminalText" class="sx-term-line is-active" :class="`is-${currentTerminalKind}`"><span>{{ currentTerminalText }}</span><i class="sx-caret" /></span><span v-else-if="terminalIsWaiting" class="sx-term-line is-active"><i class="sx-caret" /></span>
      ...
      <section id="contact" class="sx-closing">
        <ParticleSphere class="sx-close-sphere" :interactive="true" />
    ...
    <footer class="sx-footer">
      <div class="sx-container sx-footer-row">
        <div class="sx-footer-meta">
          <span>{{ homeCopy.footer.tagline }}</span>
          <span>© 2026 {{ homeCopy.brandName }}</span>
        </div>
        <a
          class="sx-tg-contact"
          href="https://t.me/JIUFENG_AI"
          target="_blank"
          rel="noopener noreferrer"
          aria-label="Contact Jiufeng on Telegram"
        >
          <img :src="tgContactImage" alt="Telegram @JIUFENG_AI" />
          <span class="sx-tg-copy">
            <small>CONTACT / TELEGRAM</small>
            <strong>@JIUFENG_AI</strong>
            <em>{{ homeCopy.footer.tgNote }}</em>
          </span>
          <span class="sx-tg-arrow" aria-hidden="true">↗</span>
        </a>
      </div>
    </footer>
  </div>
...
import tgContactImage from './jiufeng-tg.png'
import { loadLibredeskWidget } from './libredeskWidget'
import { isJiufengDark as isDark, toggleJiufengTheme } from './theme'
...
const homeCopies = {
  zh: {
    ...
    onboard: {
      ...
      terminal: [
        { kind: 'input' as TerminalLineKind, text: '$ curl -fsSL https://ai.jfengcn.com/****** | bash' },
  ...
  en: {
    ...
    footer: {
      ...
      tgNote: 'Scan to add TG - Jiufeng Support',
    },
  },
} as const
...
const introSequence = ['SIGNAL', 'MODEL MAP', 'ROUTE CORE', 'HANDSHAKE'] as const
const introRoutes = [
  { angle: -152, length: 40, delay: 0.82, color: '#6a78aa' },
  { angle: -100, length: 39, delay: 0.94, color: '#bf8658' },
  { angle: -38, length: 41, delay: 1.06, color: '#6c82bd' },
  { angle: 4, length: 39, delay: 1.18, color: '#6f6f6f' },
  { angle: 50, length: 39, delay: 1.3, color: '#837bb1' },
  { angle: 94, length: 39, delay: 1.42, color: '#b69742' },
  { angle: 142, length: 40, delay: 1.54, color: '#5c7ec2' },
  { angle: 176, length: 39, delay: 1.66, color: '#6b8076' },
] as const
const introNodes = [
  { label: 'GPT', icon: 'openai', color: '#56679b', x: 17, y: 31, delay: 1.42 },
  { label: 'CLAUDE', icon: 'claude', color: '#b9784e', x: 42, y: 11, delay: 1.55 },
  { label: 'GEMINI', icon: 'gemini', color: '#5e78b2', x: 78, y: 22, delay: 1.68 },
  { label: 'GROK', icon: 'grok', color: '#666a70', x: 90, y: 53, delay: 1.81 },
  { label: 'QWEN', icon: 'qwen', color: '#847db5', x: 73, y: 82, delay: 1.94 },
  { label: 'MISTRAL', icon: 'mistral', color: '#b09242', x: 47, y: 91, delay: 2.07 },
  { label: 'DEEPSEEK', icon: 'deepseek', color: '#5276ba', x: 19, y: 76, delay: 2.2 },
  { label: 'LLAMA', icon: 'meta', color: '#668074', x: 9, y: 53, delay: 2.33 },
] as const
const haloRoutes = [
  { angle: -150, delay: 0.08, color: '#7080b7' },
  { angle: -104, delay: 0.18, color: '#c28a62' },
  { angle: -42, delay: 0.28, color: '#6c85c5' },
  { angle: 4, delay: 0.38, color: '#73787e' },
  { angle: 48, delay: 0.48, color: '#8b83bd' },
  { angle: 94, delay: 0.58, color: '#b99b4d' },
  { angle: 140, delay: 0.68, color: '#6084c8' },
  { angle: 176, delay: 0.78, color: '#6f877d' },
] as const
const haloNodes = [
  { label: 'GPT', icon: 'openai', color: '#56679b', x: 10, y: 37, delay: 0.16 },
  { label: 'CLAUDE', icon: 'claude', color: '#b9784e', x: 28, y: 9, delay: 0.26 },
  { label: 'GEMINI', icon: 'gemini', color: '#5e78b2', x: 67, y: 9, delay: 0.36 },
  { label: 'GROK', icon: 'grok', color: '#666a70', x: 90, y: 39, delay: 0.46 },
  { label: 'QWEN', icon: 'qwen', color: '#847db5', x: 78, y: 75, delay: 0.56 },
  { label: 'MISTRAL', icon: 'mistral', color: '#b09242', x: 50, y: 91, delay: 0.66 },
  { label: 'DEEPSEEK', icon: 'deepseek', color: '#5276ba', x: 21, y: 75, delay: 0.76 },
  { label: 'LLAMA', icon: 'meta', color: '#668074', x: 5, y: 54, delay: 0.86 },
] as const
...
const ParticleSphere = defineComponent({
  ...
  setup(props) {
    ...
    const particleSeeds: ParticleSeed[] = Array.from({ length: dotCount }, (_, index) => {
      const baseY = 1 - (index / (dotCount - 1)) * 2
      const fanDirection = pseudoUnit(index, 0.58) > 0.5 ? 1 : -1
      return {
        baseY,
        ring: Math.sqrt(Math.max(0, 1 - baseY * baseY)),
        theta: index * goldenAngle,
        settleOffset: (pseudoUnit(index, 0.73) - 0.5) * 0.12,
        fanTurns: fanDirection * (Math.PI * (0.9 + pseudoUnit(index, 0.64) * 0.95)),
        jitterX: pseudoUnit(index, 0.91) - 0.5,
        jitterY: pseudoUnit(index, 0.37) - 0.5,
      }
    })
    const dots: GlobePoint[] = []
    let introStartWidth = 0
    let introStartHeight = 0
    let introStartData: Array<{ angle: number; radius: number }> = []
    let raf = 0
    ...
    function syncHoverPressure(clientX: number, clientY: number) {
      ...
      if (props.interactive) {
        targetYaw = Math.max(-1.1, Math.min(1.1, pointer.dx * 1.45))
        targetPitch = Math.max(-0.78, Math.min(0.78, -pointer.dy * 1.05))
      }
      targetPointerForce = props.interactive ? 0.68 : 0.58
    }
    ...
    function handlePointerDown(event: PointerEvent) {
      ...
      targetPointerForce = props.interactive ? 0.34 : 0.18
    ...
    function scheduleFrame() {
      if (!raf && isPageVisible && isSphereVisible) raf = requestAnimationFrame(draw)
    }

    function draw(time = 0) {
      raf = 0
      const canvas = canvasRef.value
      if (!drawContext && canvas) drawContext = canvas.getContext('2d')
      ...
      const hoverScale = props.interactive ? 1 + currentPointerForce * 0.18 : 1
      const radius = baseRadius * introScale * hoverScale
      ...
      if (props.showLabels && (width !== introStartWidth || height !== introStartHeight)) {
        introStartWidth = width
        introStartHeight = height
        introStartData = particleSeeds.map((_, index) => {
          const point = getIntroStartPoint(index, width, height)
          const dx = point.x - cx
          const dy = point.y - cy
      ...
      ctx.beginPath()
      ctx.arc(cx, cy, radius, 0, Math.PI * 2)
      ctx.globalAlpha = sceneAlpha * (props.interactive ? .34 + currentPointerForce * .2 : .16)
      ctx.strokeStyle = props.interactive ? 'rgba(74, 98, 106, .82)' : 'rgba(46, 52, 58, .5)'
      ctx.lineWidth = Math.max(1, radius * .003)
      ctx.stroke()
      ctx.globalAlpha = sceneAlpha
      ...
      function strokeSphereCurve(points: Array<[number, number, number]>, alphaScale = 1) {
        ...
        const lineReveal = props.showLabels || props.interactive ? labelProgress * sceneAlpha : reveal
        drawCtx.globalAlpha = lineReveal * alphaScale
        drawCtx.strokeStyle = props.showLabels
          ? 'rgba(18, 18, 18, .25)'
          : 'rgba(91, 112, 118, .26)'
      ...
      if ((props.showLabels || props.interactive) && labelProgress > 0.01) {
        const segments = 120
      ...
      for (let i = 0; i < dotCount; i += 1) {
        ...
        const projected = projectSpherePoint(Math.cos(seed.theta) * seed.ring, seed.baseY, Math.sin(seed.theta) * seed.ring)
        const cloudX = projected.x
        const cloudY = projected.y
        const start = props.showLabels ? introStartData[i] : null
        const settleWave = Math.max(0, Math.min(1, settleProgress + seed.settleOffset))
        ...
        const startAngle = start?.angle ?? targetAngle
        const startRadius = start?.radius ?? targetRadius
        let angleDelta = targetAngle - startAngle
        if (angleDelta > Math.PI) angleDelta -= Math.PI * 2
        if (angleDelta < -Math.PI) angleDelta += Math.PI * 2
        const spiralT = gatherProgress
        const spiralAngle = startAngle + angleDelta * spiralT + seed.fanTurns * (1 - spiralT) * spiralT
        ...
        dots.push({
          ...projected,
          x: x + seed.jitterX * jitter,
          y: y + seed.jitterY * jitter,
          index: i,
        })
      }
      ...
      for (const point of dots) {
        ...
        ctx.fillStyle = props.showLabels ? 'rgba(72, 76, 82, 1)' : 'rgba(72, 93, 100, .92)'
        ctx.beginPath()
      ...
      ctx.globalAlpha = 1
      if (props.showLabels && elapsed >= 5700) return
      scheduleFrame()
    }
    ...
    onMounted(() => {
      ...
      if (sphere) {
        ...
        isSphereVisible = rect.bottom > 0 && rect.top < window.innerHeight && rect.right > 0 && rect.left < window.innerWidth
      }

      const handleVisibilityChange = () => {
        isPageVisible = document.visibilityState !== 'hidden'
        if (!isPageVisible && raf) {
      ...
      if (sphere && 'IntersectionObserver' in window) {
        visibilityObserver = new IntersectionObserver(([entry]) => {
          isSphereVisible = entry?.isIntersecting ?? true
          if (!isSphereVisible && raf) {
            cancelAnimationFrame(raf)
            raf = 0
          }
          scheduleFrame()
...
.sx-intro-layer {
  ...
  color: #111;
  opacity: 1;
  animation: sxIntroLayerOut .92s cubic-bezier(.76, 0, .24, 1) 4.62s both;
}

.sx-intro-layer::before {
  position: absolute;
  inset: 20px;
...
.sx-intro-curtain i {
  ...
  width: 50%;
  background: #f6f7f4;
  box-shadow: 0 0 0 1px rgba(17, 17, 17, .04);
}

.sx-intro-curtain i:first-child {
  left: 0;
  animation: sxIntroCurtainLeft .98s cubic-bezier(.76, 0, .24, 1) 4.42s both;
}

.sx-intro-curtain i:last-child {
  right: 0;
  animation: sxIntroCurtainRight .98s cubic-bezier(.76, 0, .24, 1) 4.42s both;
}

.sx-intro-topline {
  position: absolute;
  top: 34px;
  right: max(52px, calc((100vw - 1120px) / 2));
  left: max(52px, calc((100vw - 1120px) / 2));
  display: flex;
...
.sx-intro-footer {
  ...
  opacity: 0;
  transform: translateY(8px);
  animation: sxIntroEdgeIn .7s cubic-bezier(.16, .84, .3, 1) .34s both;
}
...
.sx-intro-rail {
  ...
  animation: sxIntroRailIn .72s cubic-bezier(.16, .84, .3, 1) .5s both;
}
...
.sx-intro-rail-item:last-child {
  border-bottom: 1px solid rgba(17, 17, 17, .12);
}

.sx-intro-rail-item > span {
  grid-row: span 2;
  color: #b7b9b4;
  font-size: 8px;
  letter-spacing: .08em;
...
.sx-intro-rail-item > strong span {
  margin-left: 2px;
  color: #a5a8a2;
  font-size: 7px;
}

.sx-intro-rail-item > em {
  grid-column: 2 / 4;
  color: #a4a6a1;
  font-size: 7px;
  font-style: normal;
...
.sx-intro-rail-item > i {
  ...
  animation: sxIntroRailStatus 2.6s ease-in-out infinite;
}

.sx-intro-rail-right .sx-intro-rail-item {
  grid-template-columns: 1fr auto 32px;
  text-align: right;
}

.sx-intro-rail-right .sx-intro-rail-item > span,
.sx-intro-rail-right .sx-intro-rail-item > strong {
  grid-column: 1 / 3;
}
...
.sx-intro-rail-right .sx-intro-rail-item > em {
  grid-column: 1 / 3;
}

.sx-intro-rail-right .sx-intro-rail-item > i {
  right: auto;
  left: 0;
}
...
.sx-intro-center {
  ...
  animation: sxIntroCenterIn .95s cubic-bezier(.16, .84, .3, 1) .28s both;
}
...
.sx-intro-sequence::before,
.sx-intro-sequence::after {
  width: 34px;
  height: 1px;
...
.sx-intro-sequence span {
  display: inline-flex;
  align-items: center;
  gap: 5px;
  opacity: 0;
  transform: translateY(6px);
  animation: sxIntroSequenceStep 2.4s ease var(--sx-sequence-delay, .4s) both;
}

.sx-intro-sequence span:nth-child(2) { --sx-sequence-delay: 1.1s; }
.sx-intro-sequence span:nth-child(3) { --sx-sequence-delay: 1.8s; }
.sx-intro-sequence span:nth-child(4) { --sx-sequence-delay: 2.5s; }

.sx-intro-sequence span b {
  color: #b0b2ad;
  font-size: 7px;
  font-weight: 750;
}

.sx-intro-sequence span.is-active,
.sx-intro-sequence span:last-child {
  color: #565a53;
}
...
.sx-intro-network {
  ...
  border-radius: 50%;
  background: rgba(248, 248, 245, .62);
  box-shadow: 0 0 0 20px rgba(17, 17, 17, .025), 0 0 0 21px rgba(17, 17, 17, .08);
  animation: sxIntroNetworkBreath 4.8s ease-in-out .42s infinite, sxIntroNetworkBoot .9s cubic-bezier(.16, .84, .3, 1) .35s both;
}

.sx-intro-network::before,
.sx-intro-network::after {
  position: absolute;
  inset: 10%;
  border: 1px solid rgba(17, 17, 17, .12);
  border-radius: 50%;
  opacity: 0;
  content: "";
  animation: sxIntroRingIn .72s cubic-bezier(.16, .84, .3, 1) .72s both;
}
...
.sx-intro-network::after {
  ...
  animation: sxIntroRingIn .72s cubic-bezier(.16, .84, .3, 1) 1s both, sxIntroOrbit 16s linear 1.72s infinite;
}
...
.sx-intro-axis-y {
  ...
  width: 1px;
  opacity: 0;
  animation: sxIntroAxisIn .6s ease .58s both;
}

.sx-intro-network-ring {
  position: absolute;
  top: 50%;
  left: 50%;
  ...
  animation: sxIntroRingIn .72s cubic-bezier(.16, .84, .3, 1) .9s both;
}

.sx-intro-network-ring-inner {
  width: 58%;
  height: 58%;
}

.sx-intro-network-ring-outer {
  width: 80%;
  height: 80%;
  border-style: dashed;
  animation: sxIntroRingIn .72s cubic-bezier(.16, .84, .3, 1) 1.16s both, sxIntroOrbitReverse 22s linear 1.88s infinite;
}
...
.sx-intro-route {
  ...
  top: 50%;
  left: 50%;
  width: var(--sx-route-length);
  height: 1px;
  background: var(--sx-route-color);
  opacity: .44;
  transform: rotate(var(--sx-route-angle)) scaleX(0);
  transform-origin: left center;
  animation: sxIntroRouteIn .82s cubic-bezier(.16, .84, .3, 1) var(--sx-route-delay) both;
}

.sx-intro-route::after {
  position: absolute;
  top: 50%;
  right: -2px;
  width: 5px;
  height: 5px;
  border: 1px solid var(--sx-route-color);
  border-radius: 50%;
  background: #f6f7f4;
  content: "";
  transform: translateY(-50%);
...
.sx-intro-node {
  ...
  opacity: 0;
  transform: translate(-50%, -50%);
  animation: sxIntroNodeEnter .8s cubic-bezier(.16, .84, .3, 1) var(--sx-node-delay) both;
}
...
.sx-intro-node-mark {
  ...
  height: 42px;
  place-items: center;
  border: 1px solid var(--sx-node-color);
  background: #f8f8f5;
  box-shadow: 0 0 0 4px rgba(17, 17, 17, .045);
  color: var(--sx-node-color);
  animation: sxIntroNodePulse 2.8s ease-in-out calc(var(--sx-node-delay) + 1s) infinite;
}

.sx-intro-node-mark svg {
  display: block;
  width: 22px;
  height: 22px;
}
...
.sx-intro-core {
  ...
  background: #f8f8f5;
  box-shadow:
    0 0 0 8px rgba(17, 17, 17, .035),
    0 0 0 16px rgba(17, 17, 17, .018);
  opacity: 0;
  transform: translate(-50%, -50%);
  animation: sxIntroCoreIn .82s cubic-bezier(.16, .84, .3, 1) 2.62s both;
}

.sx-intro-core::before {
  position: absolute;
  inset: 11px;
...
.sx-intro-readout {
  ...
  opacity: 0;
  transform: translateY(8px);
  animation: sxIntroReadoutIn .62s cubic-bezier(.16, .84, .3, 1) 3.14s both;
}

.sx-intro-readout div {
  display: flex;
  min-height: 42px;
  flex-direction: column;
  justify-content: center;
...
.sx-intro-readout div:last-child {
  border-right: 0;
}

.sx-intro-readout span,
.sx-intro-readout strong {
  padding: 0 12px;
  font-family: var(--sx-mono);
}
...
.sx-intro-phase-label span:nth-child(1) {
  animation: sxIntroPhase 1.7s ease .46s both;
}

.sx-intro-phase-label span:nth-child(2) {
  animation: sxIntroPhase 1.7s ease 1.9s both;
}

.sx-intro-phase-label span:nth-child(3) {
...
.sx-routing-halo {
  ...
  z-index: 0;
  width: var(--sx-halo-size);
  aspect-ratio: 1;
  opacity: 0;
  transform: translate(-50%, -50%) scale(.8);
  pointer-events: none;
  animation:
    sxRoutingHaloReveal 1.28s cubic-bezier(.16, .84, .3, 1) 5.32s both,
    sxRoutingHaloBreath 6.4s ease-in-out 6.72s infinite;
}

.sx-routing-halo::before {
  position: absolute;
  top: 50%;
  left: 50%;
  width: 88%;
  height: 42%;
  border: 1px solid rgba(45, 57, 78, .34);
  border-right-color: rgba(76, 105, 153, .5);
  border-left-color: transparent;
  border-radius: 48% 52% 44% 56%;
  --sx-halo-field-angle: -14deg;
  content: "";
  opacity: 0;
  transform: translate(-50%, -50%) rotate(-14deg) scale(.82);
  animation: sxRoutingHaloFieldIn .9s cubic-bezier(.16, .84, .3, 1) 5.46s both, sxRoutingHaloFieldPulse 5.8s ease-in-out 7.4s infinite;
}

.sx-routing-halo::after {
  position: absolute;
  top: 50%;
  left: 50%;
  width: 44%;
  height: 88%;
  border: 1px dashed rgba(81, 96, 120, .34);
  border-top-color: transparent;
  border-bottom-color: rgba(194, 138, 98, .48);
  border-radius: 54% 46% 52% 48%;
  --sx-halo-field-angle: 23deg;
  content: "";
  opacity: 0;
  transform: translate(-50%, -50%) rotate(23deg) scale(.82);
  animation: sxRoutingHaloFieldIn .9s cubic-bezier(.16, .84, .3, 1) 5.72s both, sxRoutingHaloFieldDrift 13s ease-in-out 7s infinite;
}

.sx-routing-halo-field {
  position: absolute;
  top: 50%;
  left: 50%;
  border: 1px solid rgba(48, 54, 63, .26);
  border-radius: 48% 52% 42% 58%;
  opacity: 0;
  transform: translate(-50%, -50%) rotate(var(--sx-halo-field-angle)) scale(.72);
  animation: sxRoutingHaloFieldIn .82s cubic-bezier(.16, .84, .3, 1) var(--sx-halo-field-delay) both, sxRoutingHaloFieldPulse 5.4s ease-in-out calc(var(--sx-halo-field-delay) + .9s) infinite;
}

.sx-routing-halo-field.is-wide {
  width: 76%;
  height: 30%;
  --sx-halo-field-angle: 13deg;
  --sx-halo-field-delay: 5.92s;
  border-color: rgba(93, 122, 183, .38);
  border-left-color: transparent;
}

.sx-routing-halo-field.is-tall {
  width: 29%;
  height: 74%;
  --sx-halo-field-angle: -22deg;
  --sx-halo-field-delay: 6.12s;
  border-color: rgba(194, 138, 98, .34);
  border-top-color: transparent;
}

.sx-routing-halo-field.is-slice {
  width: 56%;
  height: 56%;
  --sx-halo-field-angle: 47deg;
  --sx-halo-field-delay: 6.32s;
  border-style: dashed;
  border-color: rgba(92, 105, 126, .32);
  border-right-color: transparent;
}

.sx-routing-halo-sweep {
  position: absolute;
  left: 10%;
  width: 80%;
  height: 1px;
  background: linear-gradient(90deg, transparent, rgba(92, 111, 157, .56) 36%, rgba(194, 138, 98, .5) 64%, transparent);
  opacity: 0;
  transform: translateX(-18%) rotate(var(--sx-halo-sweep-angle));
  animation: sxRoutingHaloSweep 4.6s ease-in-out var(--sx-halo-sweep-delay) infinite;
}

.sx-routing-halo-sweep.is-upper {
  top: 37%;
  --sx-halo-sweep-angle: -10deg;
  --sx-halo-sweep-delay: 6.8s;
}

.sx-routing-halo-sweep.is-lower {
  top: 64%;
  --sx-halo-sweep-angle: 12deg;
  --sx-halo-sweep-delay: 8.4s;
}

.sx-routing-halo-route {
  position: absolute;
  top: 50%;
  left: 50%;
  width: 41%;
  height: 1px;
  background: var(--sx-halo-route-color);
  opacity: 0;
  transform: rotate(var(--sx-halo-route-angle)) scaleX(0);
  transform-origin: left center;
  animation:
    sxRoutingHaloRouteIn .72s cubic-bezier(.16, .84, .3, 1) calc(5.98s + var(--sx-halo-route-delay)) both,
    sxRoutingHaloRoutePulse 3.6s ease-in-out calc(6.86s + var(--sx-halo-route-delay)) infinite;
}

.sx-routing-halo-route::after {
  position: absolute;
  top: 50%;
  right: -2px;
  width: 4px;
  height: 4px;
  border: 1px solid var(--sx-halo-route-color);
  border-radius: 50%;
  background: #fff;
  content: "";
  transform: translateY(-50%);
...
.sx-routing-halo-signal {
  ...
  transform-origin: left center;
  animation: sxRoutingHaloSignal 3.6s ease-in-out 6.72s infinite;
}

.sx-routing-halo-signal::before {
  position: absolute;
  inset: 0;
  background: linear-gradient(90deg, transparent, rgba(92, 111, 157, .55), transparent);
  content: "";
}

.sx-routing-halo-signal::after {
  position: absolute;
  top: 50%;
  right: 0;
  width: 8px;
  height: 8px;
  border: 1px solid #6175a8;
  border-radius: 50%;
  background: #fff;
  box-shadow: 0 0 0 5px rgba(97, 117, 168, .14), 0 0 18px rgba(97, 117, 168, .32);
  content: "";
  transform: translate(50%, -50%) scale(.5);
  animation: sxRoutingHaloSignalDot 1.8s ease-in-out 6.9s infinite;
}

.sx-routing-halo-signal i {
  position: absolute;
  top: 50%;
  left: 0;
  width: 6px;
  ...
  border-radius: 50%;
  background: #fff;
  box-shadow: 0 0 0 5px rgba(97, 117, 168, .16), 0 0 16px rgba(97, 117, 168, .28);
  transform: translate(-50%, -50%);
}

.sx-routing-halo-node {
  position: absolute;
  top: var(--sx-halo-node-y);
  left: var(--sx-halo-node-x);
  display: flex;
  flex-direction: column;
  align-items: center;
  gap: 5px;
  color: var(--sx-halo-node-color);
  opacity: 0;
  transform: translate(-50%, -50%) scale(.65);
  animation:
    sxRoutingHaloNodeIn .72s cubic-bezier(.16, .84, .3, 1) calc(6.18s + var(--sx-halo-node-delay)) both,
    sxRoutingHaloNodePulse 4.1s ease-in-out calc(7.12s + var(--sx-halo-node-delay)) infinite;
}

.sx-routing-halo-node b {
  display: grid;
  width: 34px;
  height: 34px;
  place-items: center;
...
.sx-routing-halo-node svg {
  width: 17px;
  height: 17px;
}

.sx-routing-halo-node em {
  color: currentColor;
  font-family: var(--sx-mono);
  font-size: 7px;
  font-style: normal;
...
.sx-routing-halo-core {
  ...
  animation:
    sxRoutingHaloCoreIn .82s cubic-bezier(.16, .84, .3, 1) 6.56s both,
    sxRoutingHaloCorePulse 4.8s ease-in-out 7.5s infinite;
}

.sx-routing-halo-core::before {
  position: absolute;
  inset: 7px;
...
.sx-routing-halo-core b,
.sx-routing-halo-core small {
  position: relative;
  z-index: 1;
}
...
.sx-home.is-intro-quick {
  --sx-button-sweep-delay: .72s;
}
...
.sx-home.is-intro-quick .sx-routing-halo {
  animation: sxRoutingHaloBreath 6.4s ease-in-out infinite;
  opacity: .3;
...
.sx-home.is-intro-quick .sx-routing-halo::before {
  animation: sxRoutingHaloFieldPulse 5.8s ease-in-out infinite;
  opacity: 1;
  transform: translate(-50%, -50%) rotate(-14deg) scale(1);
}

.sx-home.is-intro-quick .sx-routing-halo::after {
  animation: sxRoutingHaloFieldDrift 13s ease-in-out infinite;
  opacity: 1;
  transform: translate(-50%, -50%) rotate(23deg) scale(1);
}

.sx-home.is-intro-quick .sx-routing-halo-field {
  animation: sxRoutingHaloFieldPulse 5.4s ease-in-out infinite;
  opacity: 1;
  transform: translate(-50%, -50%) rotate(var(--sx-halo-field-angle)) scale(1);
}

.sx-home.is-intro-quick .sx-routing-halo-sweep {
  animation: sxRoutingHaloSweep 4.6s ease-in-out infinite;
  opacity: .8;
  transform: translateX(22%) rotate(var(--sx-halo-sweep-angle));
}

.sx-home.is-intro-quick .sx-routing-halo-signal {
  animation: sxRoutingHaloSignal 3.6s ease-in-out infinite;
  opacity: .8;
  transform: rotate(-42deg) scaleX(1);
}

.sx-home.is-intro-quick .sx-routing-halo-signal::after {
  animation: sxRoutingHaloSignalDot 1.8s ease-in-out infinite;
  opacity: 1;
  transform: translate(50%, -50%) scale(1);
}

.sx-home.is-intro-quick .sx-routing-halo-route {
  animation: sxRoutingHaloRoutePulse 3.6s ease-in-out var(--sx-halo-route-delay) infinite;
  opacity: .72;
  transform: rotate(var(--sx-halo-route-angle)) scaleX(1);
}

.sx-home.is-intro-quick .sx-routing-halo-node {
  animation:
    sxRoutingHaloNodePulse 4.1s ease-in-out var(--sx-halo-node-delay) infinite,
    sxRoutingHaloNodeDrift 5.6s ease-in-out calc(var(--sx-halo-node-delay) + .35s) infinite;
  opacity: .9;
  transform: translate(-50%, -50%) scale(1);
}

.sx-home.is-intro-quick .sx-routing-halo-core {
  animation: sxRoutingHaloCorePulse 4.8s ease-in-out infinite;
  opacity: .62;
  transform: translate(-50%, -50%) scale(1);
}
...
.sx-hero-slogan {
  ...
  margin: var(--sx-hero-bottom-gap) 0 0;
  font-family: var(--sx-serif);
  font-size: 23px;
  font-weight: 800;
  line-height: 1.35;
...
.sx-primary::before {
  ...
  z-index: 0;
  border-radius: 999px;
  background: linear-gradient(104deg, transparent 30%, rgba(255, 255, 255, .16) 40%, rgba(255, 255, 255, .72) 48%, rgba(255, 255, 255, .98) 51%, rgba(255, 255, 255, .2) 58%, transparent 69%);
  opacity: 0;
  transform: translateX(-78%) skewX(-14deg);
  pointer-events: none;
  animation: sxButtonSweep 3.8s cubic-bezier(.42, 0, .22, 1) var(--sx-button-sweep-delay) infinite;
}
...
.sx-close-sphere:hover {
  transform: scale(1.035);
}

.sx-close-sphere:active {
  cursor: grabbing;
}
...
.sx-tg-contact {
  ...
  border-radius: 12px;
  background: linear-gradient(135deg, #fff, #f7f6f2);
...
.sx-tg-contact:hover .sx-tg-arrow,
.sx-tg-contact:focus-visible .sx-tg-arrow {
  color: #111;
...
@keyframes sxIntroCurtainLeft {
  from { transform: translateX(0); }
  to { transform: translateX(-100%); }
}

@keyframes sxIntroCurtainRight {
  from { transform: translateX(0); }
  to { transform: translateX(100%); }
}
...
@keyframes sxIntroRailIn {
  from {
    opacity: 0;
    transform: translateY(-43%) translateX(var(--sx-rail-shift, 18px));
  }
  to {
    opacity: 1;
...
@keyframes sxIntroRailStatus {
  0%, 100% { opacity: .42; transform: translateY(-50%) scale(.86); }
  50% { opacity: 1; transform: translateY(-50%) scale(1.2); }
}

@keyframes sxIntroSequenceStep {
  0%, 100% {
    opacity: 0;
    transform: translateY(6px);
  }
...
@keyframes sxIntroRingIn {
  from { opacity: 0; }
  to { opacity: 1; }
}

@keyframes sxIntroAxisIn {
  from { opacity: 0; }
  to { opacity: .24; }
}

@keyframes sxIntroCoreIn {
  from {
    opacity: 0;
    transform: translate(-50%, -50%) scale(.72);
  }
...
@keyframes sxIntroNetworkBreath {
  0%, 100% { transform: scale(1); }
  50% { transform: scale(1.012); }
}

@keyframes sxIntroOrbit {
  to { transform: translate(-50%, -50%) rotate(360deg); }
}

@keyframes sxIntroOrbitReverse {
  to { transform: translate(-50%, -50%) rotate(-360deg); }
}

@keyframes sxIntroRouteIn {
  from { transform: rotate(var(--sx-route-angle)) scaleX(0); }
  to { transform: rotate(var(--sx-route-angle)) scaleX(1); }
}

@keyframes sxIntroNodeEnter {
  from {
    opacity: 0;
...
@keyframes sxIntroNodePulse {
  ...
  50% {
    background: #fff;
    box-shadow: 0 0 0 8px rgba(17, 17, 17, .07);
...
@keyframes sxRoutingHaloFieldIn {
  from {
    opacity: 0;
    transform: translate(-50%, -50%) rotate(var(--sx-halo-field-angle)) scale(.72);
  }
...
@keyframes sxRoutingHaloFieldPulse {
  0%, 100% { opacity: .72; }
  50% { opacity: 1; }
}

@keyframes sxRoutingHaloFieldDrift {
  0%, 100% {
    transform: translate(-50%, -50%) rotate(23deg) scale(1);
  }
  50% {
    transform: translate(-50%, -50%) rotate(29deg) scale(1.035);
  }
}

@keyframes sxRoutingHaloSweep {
  0%, 100% {
    opacity: 0;
    transform: translateX(-22%) rotate(var(--sx-halo-sweep-angle));
  }
...
@keyframes sxRoutingHaloRouteIn {
  from {
    opacity: 0;
    transform: rotate(var(--sx-halo-route-angle)) scaleX(0);
  }
...
@keyframes sxRoutingHaloRoutePulse {
  0%, 100% { opacity: .72; }
  50% { opacity: 1; }
}

@keyframes sxRoutingHaloNodeIn {
  from {
    opacity: 0;
    transform: translate(-50%, -50%) scale(.65);
  }
...
@keyframes sxRoutingHaloNodePulse {
  0%, 100% {
    opacity: .9;
  }
  50% {
...
@keyframes sxRoutingHaloNodeDrift {
  0%, 100% {
...
@keyframes sxRoutingHaloCorePulse {
  0%, 100% {
    box-shadow: 0 0 0 8px rgba(17, 17, 17, .045), 0 0 0 16px rgba(17, 17, 17, .018);
  }
  50% {
    box-shadow: 0 0 0 12px rgba(82, 102, 139, .1), 0 0 0 24px rgba(82, 102, 139, .035);
  }
}

@keyframes sxRoutingHaloSignal {
  0%, 100% { opacity: 0; transform: rotate(-42deg) scaleX(.4); }
  18%, 76% { opacity: .8; transform: rotate(-42deg) scaleX(1); }
}

@keyframes sxRoutingHaloSignalDot {
  0%, 100% {
    opacity: .3;
    transform: translate(50%, -50%) scale(.5);
  }
