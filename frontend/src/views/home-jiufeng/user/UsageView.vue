<template>
  <AppLayout>
    <div class="jf-chart-palette space-y-6">
...
const EXPORT_MAX_RETRIES = 3

const isRetryableExportError = (error: unknown): boolean => {
  const requestError = error as { code?: string; response?: { status?: number } }
  const status = requestError.response?.status
  return !status || status === 429 || status >= 500 || requestError.code === 'ECONNABORTED'
}

const queryExportPage = async (
  page: number,
  exportParams: ReturnType<typeof buildUsageListParams>,
): Promise<Awaited<ReturnType<typeof usageAPI.query>>> => {
  for (let attempt = 0; attempt < EXPORT_MAX_RETRIES; attempt += 1) {
    try {
      return await usageAPI.query({ ...exportParams, page })
    } catch (error) {
      if (!isRetryableExportError(error) || attempt === EXPORT_MAX_RETRIES - 1) throw error
      await new Promise((resolve) => window.setTimeout(resolve, 400 * (attempt + 1)))
    }
  }
  throw new Error('Usage export request did not complete')
}
...
const exportToCSV = async () => {
  ...
  try {
    ...
    const csvChunks = [headers.map(escapeCSVValue).join(',')]
    let page = 1
    let exportedRows = 0
    while (true) {
      const response = await queryExportPage(page, exportParams)
      ...
      if (rows.length > 0) csvChunks.push(rows.join('\n'))
      exportedRows += response.items.length
...
const loadFilterOptions = async () => {
  try {
    const apiKeyPageSize = 100
    const allApiKeys: ApiKey[] = []
    let page = 1
    let totalPages = 1

    // The API-key endpoint is paginated. Load every page so the usage filter
    // can select keys created before the first 100 entries.
    const loadAllApiKeys = async () => {
      while (page <= totalPages) {
        const response = await keysAPI.list(page, apiKeyPageSize)
        allApiKeys.push(...(response.items || []))

        const pagesFromTotal = response.total
          ? Math.ceil(response.total / apiKeyPageSize)
          : 0
        totalPages = Math.max(response.pages || 0, pagesFromTotal, 1)
    ...
    const [loadedApiKeys, availableGroups] = await Promise.all([
      loadAllApiKeys(),
