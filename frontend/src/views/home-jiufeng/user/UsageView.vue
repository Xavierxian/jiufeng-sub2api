...
const queryExportPage = async (page: number): Promise<Awaited<ReturnType<typeof usageAPI.query>>> => {
  for (let attempt = 0; attempt < EXPORT_MAX_RETRIES; attempt += 1) {
    try {
      return await usageAPI.query(buildUsageListParams(page, EXPORT_PAGE_SIZE))
    } catch (error) {
...
const exportToCSV = async () => {
  ...
  try {
    ...
    let exportedRows = 0
    while (true) {
      const response = await queryExportPage(page)
