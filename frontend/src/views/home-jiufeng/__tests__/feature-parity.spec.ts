...
describe('Jiufeng feature parity', () => {
  ...
  it('uses the current v0.2 reasoning effort policy implementation', () => {
    const groups = read('admin/GroupsView.vue')
    const fields = read('admin/group/ReasoningEffortPolicyFields.vue')
    expect(groups).toContain('@/views/admin/groupsReasoningEffort')
    expect(groups).toContain('@/components/admin/group/ReasoningEffortPolicyFields.vue')
    expect(fields).toContain('@/views/admin/groupsReasoningEffort')
    expect(read('admin/groupsReasoningEffort.ts')).toContain('@/views/admin/groupsReasoningEffort')
  })

  it('keeps v0.2 controls and pricing fields in Jiufeng views', () => {
    const groups = read('admin/GroupsView.vue')
    const channels = read('admin/ChannelsView.vue')
    const adminUsage = read('admin/UsageView.vue')
    const userUsage = read('user/UsageView.vue')
    ...
    expect(groups).toContain('force_openai_fast')
    expect(groups).toContain('free_openai_fast')
    expect(groups).toContain('max_reasoning_effort_over_limit')
    expect(groups).toContain('cache_write_1h_price')
    expect(channels).toContain('cache_write_1h_price')
    expect(adminUsage).toContain('native_compaction_v2')
    expect(userUsage).toContain('native_compaction_v2')
    expect(register).toContain(':promo-code="formData.promo_code"')
    expect(register).toContain('emailDomainQuotaEnabled')
    expect(settings).toContain('openai_ttft_mode')
  })
  ...
  it('includes complete v0.1.171 admin surfaces', () => {
    ...
    expect(reasoning).toContain('export * from "@/views/admin/groupsReasoningEffort"')
  ...
  it('includes complete v0.1.175 frontend capabilities', () => {
    ...
    expect(usage).toContain("const HIDDEN_COLUMNS_CURRENT_VERSION = 'upstream-request-id-hidden-by-default'")
  ...
  it('includes complete v0.1.180 plugin management surfaces', () => {
    const plugins = read('admin/PluginsView.vue')
    const settings = read('admin/SettingsView.vue')
    const sidebar = read('JFAppSidebar.vue')
    const router = read('../../router/index.ts')

    expect(plugins).toContain('<JFAppLayout>')
    expect(plugins).toContain('adminAPI.plugins.upload')
    expect(plugins).toContain('adminAPI.plugins.createUISession')
    expect(plugins).toContain('pluginStepUp.run')
    expect(settings).toContain('v-model="form.plugin_management_enabled"')
    expect(settings).toContain('plugin_management_enabled: form.plugin_management_enabled')
    expect(sidebar).toContain("path: '/admin/plugins'")
    expect(sidebar).toContain('FeatureFlags.pluginManagement')
    expect(router).toContain("path: '/admin/plugins'")
    expect(router).toContain("name: 'AdminPlugins'")
    expect(router).toContain("import('@/views/home-jiufeng/admin/PluginsView.vue')")
  })

  it('includes complete v0.1.180 group, proxy, account, and home behavior', () => {
    const groups = read('admin/GroupsView.vue')
    const messagesDispatch = read('admin/groupsMessagesDispatch.ts')
    const proxies = read('admin/ProxiesView.vue')
    const accounts = read('admin/AccountsView.vue')
    const home = read('JFHomeView.vue')

    expect(groups).toContain('supportsMessagesDispatchPlatform(createForm.platform)')
    expect(groups).toContain('supportsMessagesDispatchPlatform(editForm.platform)')
    expect(messagesDispatch).toContain('platform === "openai" || platform === "composite"')
    expect(proxies).toContain('rawHost')
    expect(proxies).toContain("rawHost.replace(/^\\[|\\]$/g, '')")
    expect(accounts).toContain("const DEFAULT_HIDDEN_COLUMNS = ['today_stats', 'proxy', 'notes', 'scheduler_score', 'rate_multiplier']")
    expect(home).toContain('showModelPlazaEntry')
    expect(home).toContain('to="/model-plaza"')
  })

  it('includes complete v0.1.180 operations diagnostics and list restoration', () => {
    const dashboard = read('admin/ops/OpsDashboard.vue')
    const errorList = read('admin/ops/components/OpsErrorDetailsModal.vue')
    const requestList = read('admin/ops/components/OpsRequestDetailsModal.vue')
    const detail = read('admin/ops/components/OpsErrorDetailModal.vue')

    expect(dashboard).toContain(':resume-state="resumeListState"')
    expect(dashboard).toContain('detailReturnTarget.value = showRequestDetails.value')
    ...
    expect(detail).toContain('detail.upstream_status_code')
    expect(detail).toContain('rootCauseMessage')
    expect(detail).toContain('diagnosticPayloadSections')
    expect(detail).toContain('data-testid="error-detail-back-to-list"')
    expect(detail).toContain('detail.attempted_key_prefix')
  })

  it('includes complete v0.2.2 admin feature parity fixes', () => {
    const channels = read('admin/ChannelsView.vue')
    const groups = read('admin/GroupsView.vue')
    const accounts = read('admin/AccountsView.vue')
    const usage = read('admin/UsageView.vue')
    ...
    expect(channels.match(/max_reasoning_effort_multiplier/g)?.length).toBeGreaterThanOrEqual(5)
    expect(groups).toContain('CodexManifestAccountsField')
    expect(groups).toContain('codex_models_manifest_config:')
    expect(groups).toContain('createCodexManifestDefaults')
    expect(groups).toContain('getModelAllowlistCandidates')
    expect(groups).toContain('model_allowlist: buildModelsListConfig')
    expect(groups).toContain('addCustomModelsListItem')
    expect(groups).toContain('admin.groups.modelAllowlist.emptySelectionError')
    ...
    expect(groups).not.toContain('getModelsListCandidates')
    expect(groups).not.toContain('models_list_config')
    expect(groups).not.toContain('admin.groups.modelsList')
    expect(sidebar).toContain("{ path: '/admin/groups', label: t('nav.groups'), icon: FolderIcon }")
    expect(subscriptions).toContain("query: { user_id: row.user_id }")
    expect(accounts).toContain("lite: '1'")
    expect(accounts).toContain('accountGroupsForRow(row)')
    expect(accounts).toContain('adminAPI.accounts.getById(account.id)')
    expect(usage).toContain("{ key: 'upstream_request_id'")
    expect(usage).toContain("log.upstream_request_id || ''")
    expect(settings).toContain('value: "ultrafast"')
  })

  it('includes complete v0.2.4 frontend parity fixes', () => {
    const accounts = read('admin/AccountsView.vue')
    const channels = read('admin/ChannelsView.vue')
    const groups = read('admin/GroupsView.vue')
    const settings = read('admin/SettingsView.vue')
    ...
    const announcements = read('admin/AnnouncementsView.vue')
    const systemLogs = read('admin/ops/components/OpsSystemLogTable.vue')
    const monitor = read('user/ChannelStatusV2View.vue')
    const monitorFormat = read('features/channel-monitor-v2/monitorFormat.ts')
    const metricCell = read('features/channel-monitor-v2/MetricCell.vue')
    const customPage = read('user/CustomPageView.vue')
    const payment = read('user/PaymentView.vue')
    const login = read('JFLoginView.vue')

    expect(accounts).toContain(':anchor-rect="menu.anchorRect"')
    expect(accounts).toContain('const accountIds = [...selIds.value]')
    expect(accounts).toContain('const failedIds = result.errors?.map(error => error.account_id)')
    expect(channels).toContain("'minimax'")
    expect(groups).toContain("value === 'minimax'")
    expect(settings).toContain('v-model="form.channel_monitor_hide_user_ranking"')
    expect(settings).toContain('channel_monitor_hide_user_ranking: Boolean(form.channel_monitor_hide_user_ranking)')
    expect(proxies).toContain('max="9999-12-31"')
    expect(proxies).toContain("case 'minimax':")
    expect(announcements).toContain('max="9999-12-31T23:59"')
    expect(systemLogs).toContain('runtimeConfig.persist_access_logs')
    expect(systemLogs).toContain('admin.ops.systemLogs.persistAccessLogs')
    expect(monitor).toContain('isChannelMonitorUserRankingHidden')
    expect(monitor).toContain('ttftCellState(snapshot.health.ttft, snapshot.metrics.ttft)')
    expect(monitorFormat).toContain('export function ttftDisplayState(')
    expect(metricCell).toContain('const missingValue = computed')
    expect(customPage).toContain('@pointerdown="startButtonDrag"')
    expect(payment).toContain('DOMPurify.sanitize')
    expect(login).toContain('publicSettingsLoaded && registrationEnabled')
  })
})
