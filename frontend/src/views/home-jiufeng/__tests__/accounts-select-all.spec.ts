...
const DataTableStub = {
  ...
  template: `
    <div data-test="data-table">
      ...
      <slot v-if="data.length" name="cell-actions" :row="data[0]" />
    </div>
  `,
}
...
const mountView = () => mount(AccountsView, {
  ...
  global: {
    stubs: {
      ...
      AccountActionMenu: {
        props: ['show'],
        template: '<div v-if="show" class="action-menu-content"><div data-test="menu-scroll-content" /></div>',
      },
...
describe('Jiufeng accounts v0.1.170 behavior', () => {
  ...
  it('keeps the action menu open for internal scrolling and closes it on page scroll', async () => {
    const items = makeAccounts(1)
    listAccounts.mockResolvedValue({ items, total: 1, page: 1, page_size: 20, pages: 1 })
    const wrapper = mountView()
    try {
      await flushPromises()
      await wrapper.findAll('button').find(button => button.text().includes('common.more'))!.trigger('click')
      expect(wrapper.find('.action-menu-content').exists()).toBe(true)
      await wrapper.get('[data-test="menu-scroll-content"]').trigger('scroll')
      expect(wrapper.find('.action-menu-content').exists()).toBe(true)
  ...
  it('refreshes compact billing data after a bulk billing probe updates a snapshot', async () => {
    ...
    getUpstreamBillingRatesWithEtag.mockResolvedValue({
      ...
      data: {
        items: [{
          ...
          snapshot: {
            status: 'ok',
            probed_at: '2026-08-03T00:00:00Z',
    ...
    expect(listAccounts).toHaveBeenCalledTimes(callsBeforeProbe)
    expect(wrapper.text()).toContain('0.065x')
