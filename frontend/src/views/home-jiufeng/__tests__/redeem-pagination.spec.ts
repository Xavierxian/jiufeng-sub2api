...
import JFRedeemView from '../JFRedeemView.vue'
import UserRedeemView from '../user/RedeemView.vue'
...
describe.each([['Jiufeng', JFRedeemView], ['user copy', UserRedeemView]] as const)('%s redemption', (_name, RedeemView) => {
async function submitCode() {
  const wrapper = mount(RedeemView, {
    global: { stubs: { AppLayout: { template: '<div><slot /></div>' }, Icon: true, transition: { template: '<div><slot /></div>' } } },
  })
  await flushPromises()
...
describe('RedeemView refresh after redemption', () => {
  ...
  it.each(['balance', 'concurrency', 'subscription'])(
    'keeps a successful %s redemption when profile refresh fails', async (type) => {
      ...
      expect(wrapper.text()).toContain(RedeemView === JFRedeemView ? 'REDEEM-CODE' : 'REDEEM-C...')
  ...
  it('renders every record in the requested page, including rows beyond the old eight-row preview', async () => {
    getHistory.mockResolvedValue({
      items: Array.from({ length: 20 }, (_, index) => ({
        id: index + 1, code: `ENTRY-${String(index + 1).padStart(3, '0')}`, type: 'balance', value: 1,
        used_at: '2026-03-08T00:00:00Z',
      })),
      total: 45,
    })
    const wrapper = await submitCode()
    expect(wrapper.text()).toContain(RedeemView === JFRedeemView ? 'ENTRY-020' : 'ENTRY-02...')
    expect(getHistory).toHaveBeenLastCalledWith(1, 20)
    wrapper.unmount()
  })
