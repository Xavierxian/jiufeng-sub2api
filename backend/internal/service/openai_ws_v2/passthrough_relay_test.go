...
func TestRelay_NoSemanticOutputTerminalSequence_RecordsLegacyFirstToken(t *testing.T) {
	t.Parallel()
...
func TestRelay_NoDeltaOutputDoneEvent_UsesLegacyTokenBoundary(t *testing.T) {
	t.Parallel()

	tests := []struct {
		name               string
		donePayload        string
		wantBeforeTerminal bool
	}{
		{
			name:               "output text done",
			donePayload:        `{"type":"response.output_text.done","response_id":"resp_done","text":"hello"}`,
			wantBeforeTerminal: true,
		},
		{
	...
	for _, tt := range tests {
		...
		t.Run(tt.name, func(t *testing.T) {
			...
			if tt.wantBeforeTerminal {
				...
				require.Equal(t, turn.Duration.Milliseconds(), int64(*turn.FirstTokenMs))
